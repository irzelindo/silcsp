...
$mensage=0;
                 ...
                 $sql = 'select * from medicos order by codmedico ';
                 ...
                 if ($numeroRegistros<=0)
                 ...
                 else
                    {
                   	if ($tiporeporte==1) // reporte HTML
             	    {
                        ...
                        echo '<p style="margin-left:0px"><strong>&nbsp;REPORTE DE TABLA DE M&Eacute;DICOS</strong></p></td>';
    	                echo '</tr></table>';
                        ...
                        print '<br><table class="cuadro">'
    		                  ...
    		                  .'<td class="hr">Nro. Registro</td>'
                              .'<td class="hr">Tipo Profesional</td>'
                        ...
                        while ($row = pg_fetch_array($res))
                              ...
                              $estado=$row['estado'];
                              $estadox='';
                              if($estado==1){$estadox=' Activo';}else{if($estado==2){$estadox=' Inactivo';}}
     		                  print '<tr>'
                                   .'<td class="tr" width="80px">'.$row['codmedico'] .'</td>'
    		                       .'<td class="tr" width="80px">'.$row['nroregistro'] .'</td>'
                                   .'<td class="tr" width="300px">'.$row['tipoprof'] .'</td>'
                                   .'<td class="tr" width="300px">'.$row['nomyape'] .'</td>'
                                   .'<td class="tr" width="80px">'.$estadox .'</td>'
                                   .'</tr>';
    	                       }
    	                 print '</table><br />';
					 ...
					 else // EXCEL O PDF
                     {
                          ...
                          print  '<table border=1>'
    	                        .'<tr><td style="font-weight:bold;font-size:1em;background-color:#F1F1F1;color:black;" colspan="5">REPORTE DE TABLA DE MEDICOS</td></tr>';
                          print '</table>';		
    	
                          print  '<table border=1>';	
                          print  '<tr style="background-color:#F1F1F1;color:black;">'	
                                .'<td style="font-size:1em; width:80px;font-weight: bold;">C&oacute;digo</td>'
    	                        .'<td style="font-size:1em; width:80px;font-weight: bold;">Nro. Registro</td>'
                                .'<td style="font-size:1em; width:300px;font-weight: bold;">Tipo Profesional</td>'
                                .'<td style="font-size:1em; width:300px;font-weight: bold;">Nombre y Apellido</td>'
                                .'<td style="font-size:1em; width:80px;font-weight: bold;">Estado</td>'
                                .'</tr>';
                        while ($row = pg_fetch_array($res))
    	                      {
                              $estado=$row['estado'];
                              ...
                              if($estado==1){$estadox=' Activo';}else{if($estado==2){$estadox=' Inactivo';}}
     		                  print '<tr>'
    		                       ...
    		                       .'<td>'.acentos($row['tipoprof']).'</td>'
                                   .'<td>'.acentos($row['nomyape']).'</td>'
                                   .'<td>'.$estadox.'</td>'
    		                       .'</tr>';
    	                       }
    	                 print '</table>';
                         ...
                         $fechaxx=date("Y-n-j", time());
                         $hora=date("G:i:s",time());
                         $accion="Reporte Tabla Medico XLS ";
                         $terminal = $_SERVER['REMOTE_ADDR'];
                         $a=archdlog($codusu,$codopcx,$fechaxx,$hora,$accion,$terminal);
                         // Fin grabacion de registro de auditoria	
                     }	// FIN EXCCEL
