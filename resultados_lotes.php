	...
	<script>
		...
		$( function () {
			...
			jQuery( "#listapacientes" ).jqGrid( {
				url: 'datosresultadolote.php?codsector=<?php echo $codsector; ?>&codestudio=<?php echo $codestudio; ?>&desde=<?php echo $desde; ?>&hasta=<?php echo $hasta; ?>',
				datatype: 'json',
				mtype: 'GET',
				loadonce: true,
				height: 330,
				...
				colNames: [ 'Nro. Agrupado', 'Nro. Orden', 'Estudio', 'Nombres y Apellidos', 'Tipo Muestra', 'Actual Establecimiento', '', '', '', '', '', ''  ],
				colModel: [ {
						name: 'grupo',
						index: 'grupo',
						width: 120,
      ...
      var grupo        = jQuery( "#grupo" ).val();
			...
			jQuery( "#listapacientes" ).jqGrid( 'setGridParam', {
				url: "datosresultadolote.php?codsector=" + codsector + "&codestudio=" + codestudio + "&desde=" + desde + "&hasta=" + hasta + "&grupo=" + grupo,
				datatype: 'json'
			} ).trigger( "reloadGrid" );
		...
		function GenerarAgrupamiento()
		{
			var codsector     = jQuery( "#codsector" ).val();
			var codestudio    = jQuery( "#codestudio" ).val();
			var codresultado  = jQuery( "#codresultado1" ).val();

			var s = jQuery("#listapacientes").jqGrid('getGridParam','selarrrow'), selnordentra = [], selmicrobiologia = [];
			...
			if(cantidad != 0)
			{
						$.each(s,function()
						{
							...
							selmicrobiologia.push(jQuery("#listapacientes").jqGrid('getCell',this,'microbiologia'));

						});

						jQuery.ajax({
								url:'nuevo_agrupadolote.php',
								type:'POST',
								dataType:'json',
								data:{codsector:codsector, codestudio:codestudio, nordentra:selnordentra, microbiologia:selmicrobiologia, codresultado:codresultado, cantidad:cantidad}
								...
								if(respuesta.grupo != 0)
								{
									...
									$.ajax({
										type: 'POST',
										dataType: "json",
										url: 'http://dgvs.mspbs.gov.py/sistemas/itdgvsops/dataserver/ajax/actualizar/actLCSP-2',
										data:{ actualizar: respuesta.resultados },
									}).done(function(data) {
										console.log(data.respuesta.error);
										if ( data.respuesta.error == "" ) {
											for (var i = 0; i < data.respuesta.respuesta.length; i++) {
												if ( data.respuesta.respuesta[i]['cargo'] == true ) {
													logros += "Orden: "+data.respuesta.respuesta[i]['orden']+" Actualizado correctamente en la D.G.V.S."+"\n";
												}else{
													logros += "Orden: "+data.respuesta.respuesta[i]['orden']+" Ya tiene precargado el resultado favor comunicar a la D.G.V.S."+"\n";
												}
											}
										}
										else
										{
											logros = data.respuesta.error;
										}
										...
										setTimeout(function () {
											swal({
													...
													window.location = "resultados_fecha.php";
										 		});
										}, 1000);
									 ...
									 setTimeout(function () {
									 	swal({
									 		...
									 		//window.open('impresion_lista_orden.php?grupo='+respuesta.grupo, '_blank');
									 		window.location = "resultados_lotes.php";
									  	});
									 }, 1000);
		...
		function ListaResultado()
		{
			...
			var data = {};
			$("#listaresultado option").each(function(i,el) {
			   data[$(el).data("value")] = $(el).val();
			});
			console.log(data, $("#listaresultado option").val());

				var value = $('#codresultado').val();

				$('#codresultado1').val($('#listaresultado [value="' + value + '"]').data('value'));
...
<body onload="gridReload()">
				...
				<div class="row">
					<div id="page1Layer1_Container">
						...
						if ( $v_211 == 2 || $v_211 == 3 ) {
							...
							echo '<button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#myModal" id="button">Procesar Lote</button>';
	...
	<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" >
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<input type="text" name="codresultado" id="codresultado" list="listaresultado" style="width: 80%; margin-top: 10px;" onChange="ListaResultado()" autofocus>

				 <input type="hidden" name="codresultado1" id="codresultado1" value="<?php echo $codresultado; ?>" >

			</div>
			<div class="modal-footer">
			<button type="button" class="btn btn-primary" onclick="GenerarAgrupamiento()">Guardar Datos</button>
			</div>
		</div>
		</div>
	</div>
	...
	<datalist id="listaresultado">
		<?php
			$tabla_dpto = pg_query($link, "select * from resultadocodificado");
			echo '<option data-value = "" value = ""></option>';
			while($depto = pg_fetch_array($tabla_dpto))
			{
				echo '<option data-value = "'.$depto['codresultado'].'" value = "'.$depto['codresultado']."- ".utf8_decode($depto['nomresultado']).'"></option>';
			}
	?>
	</datalist>
