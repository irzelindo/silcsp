...
@Header("Content-type: text/html; charset=utf-8");
   ...
   $sql1 = "select * from sintomasnob where codsintoma = '$id' and tipo=$tipo"; 
   ...
   if($numeroRegistros<=0)
      {
      $resultado1=pg_query($con,"DELETE FROM sintomas WHERE codsintoma = '$id'  and tipo=$tipo"); 
      ...
      if($resultado1==true)
           {
              ...
              $accion="Sintomas: Elimina-Reg.: ".$id;
              $terminal = $_SERVER['REMOTE_ADDR'];
              $a=archdlog($codusu,$codopc,$fecha,$hora,$accion,$terminal);
              
              // Fin grabacion de registro de auditoria
              header("Location: sintomas.php?mensage=1");	
           }
    	else
    	  {
      ...
      {		
      header("Location: sintomas.php?mensage=4"); //no se puede borrar
      }
?>
