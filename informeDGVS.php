...
<script type="text/javascript">
	...
	function Buscar(argument) {
		if ($("#FechaResLabD").val() != "" && $("#FechaResLab").val() != "") {
			...
			$.ajax({
				type: 'POST',
				dataType: "json",
				url: 'informeDGVSajax.php',
				data:{ FechaD: $("#FechaResLabD").val(), FechaH: $("#FechaResLab").val(), op: argument },
			}).done(function(data) {
   ...
   function MatrizInformeDGVS(argument,argument1,callback) {
		...
		if ( ArrArgument.length > 0 ) {
			...
			var llaves = $.map(ArrArgument[0], function(element,index) {return "<th>"+index+"</th>"});
			var Cabeza = llaves.join("");

			$($newtablehead).append(Cabeza);
			$($newtable).append($newtablehead);

			var $newtablebody = $("<tbody>");
			for (var z = 0; z < ArrArgument.length; z++) {
				var valores = $.map(ArrArgument[z], function(element,index) {return "<td>"+element+"</td>"});
				$newtr = "<tr>"+valores.join("")+"</tr>";
			...
			$("#tsw"+argument1).append(   '<hr style="border-top: 1px solid black;margin-bottom: 0">'+
			                     '<h3 class="block-title" style="padding-left: 15px;margin-top:10px;background-color: darkorange;color: white;"> Informe a D.G.V.S.</h3>'+
...
<!-- EMPIEZA -->
   <div class="col-xs-12" id="frm_fsarscov2" name="frm_fsarscov2" style="margin-top: 0px;padding: 0px;">
      <div class="page-header" style="margin-top: 0px;padding-top: 0px;">
         <h3 style="margin-top: 0px;padding-top: 0px;">Informe D.G.V.S.</h3>
      </div>
<!--       <div class="row">
         <div calss="col-sm-12">
