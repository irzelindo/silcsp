...
@Header("Content-type: text/html; charset=utf-8");
   ...
   $_SESSION['coddeterminax']=$_POST['coddeterminax'];
   ...
   $coddeterminax = trim($_POST['coddeterminax']);
    ...
    if ($nroreg1==0 || ($nroreg1>0 && $coddetermina==$coddeterminax))
	   {
        ...
        $tiempourg=$registro1['tiempourg'];            
	 	...
	 	$result = pg_query($conn, "UPDATE determinaciones set coddetermina='$coddetermina',
                                                              nomdetermina='$nomdetermina',
                                                              ...
                                                              tiempohab=$tiempohab, 
                                                              tiempourg=$tiempourg
                                           where codestudio = '$codestudio' and coddetermina = '$coddeterminax'"); 

        //  ----- Ahora ELIMINO primero y luego Copio datos del MASTER de RANGOS y RESULTADOS POSIBLES -----//
        $resultado2=pg_query($conn,"DELETE FROM determinacionrango WHERE codestudio = '$codestudio'  and coddetermina = '$coddeterminax' ");  
        ...
        //---------------------------------------------------------------------------//
        $resultado3=pg_query($conn,"DELETE FROM resultadoposible WHERE codestudio = '$codestudio'  and coddetermina = '$coddeterminax' ");  

        $sql2 = "select * from resultadoposible where coddetermina='$coddetermina' order by codresultado ";
        ...
        $accion="Determinaciones de Estudios: Modifica-Reg.: ".$codestudio.', Cod.Determinacion: '.$coddetermina;
        $terminal = $_SERVER['REMOTE_ADDR'];
        $a=archdlog($_SESSION['codusu'],$codopc,$fecha1,$hora,$accion,$terminal);
        // Fin grabacion de registro de auditoria
  
   		header("Location: modifica_estudios.php?mensage=7&id=".$codestudio);
       }
	else
       {
   		header("Location: modifica_determinacionrel.php?mensage2=1&codestudio=$codestudio&coddetermina=$coddeterminax"); 
       }
?>
