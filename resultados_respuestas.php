...
$v_181   = $_SESSION['V_181'];	// Realizar Evaluacion
	...
	<script>
		$(function() {
			...
			jQuery("#listapacientes").jqGrid({
				url:'datosresultadosrespuestas.php?nroeval=<?php echo $nroeval; ?>&mes=<?php echo $mes; ?>&anio=<?php echo $anio; ?>&usuario=<?php echo $usuario; ?>',
				datatype: 'json',
				mtype: 'GET',
				loadonce:true,
				height: 330,
				...
				colNames:['Revisar','Imprimir','Descarga','Nro. Evaluaci&oacute;n', 'Sector', 'SubPrograma', 'Mes', 'A&ntilde;o', 'Usuario'],
				colModel:[
	
						{name:'modifica', width:70, resizable:false, align:"center", editable: false, editoptions:{maxlength:"50"}, search: false},
					
					    {name:'pdf', width:70, resizable:false, align:"center", editable: false, editoptions:{maxlength:"50"}, search: false},
					
						{name:'descarga', width:70, resizable:false, align:"center", editable: false, editoptions:{maxlength:"50"}, search: false},
						...
						{name:'nomsector',index:'nomsector', width:120, align:"center", editable: true, searchoptions: {attr: {maxlength: 10,size: 50,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
					
						{name:'subprograma',index:'subprograma', width:120, align:"center", editable: true, searchoptions: {attr: {maxlength: 10,size: 50,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
		...
		function gridReload() {
			...
			jQuery( "#listapacientes" ).jqGrid( 'setGridParam', {
				url: "datosresultadosrespuestas.php?nroeval=" + nroeval + "&mes=" + mes + "&anio=" + anio + "&usuario=" + usuario,
				datatype: 'json'
			} ).trigger( "reloadGrid" );
		}
		
		function abrirVentana(nroeval, codusu)
		{
			window.location = 'evaluacion_examen.php?nroeval='+nroeval+'&usuario='+codusu;
		}
		
		function abrirVentana1(nroeval, codusu)
		{
			window.location = 'imprimir_evaluacion_final.php?nroeval='+nroeval+'&usuario='+codusu;
		}
		
		function openWin(nroeval)
		{

			$.ajax({

				url: "upload/Evaluacion_"+nroeval+".pdf",
				type: 'HEAD',
				error: function () {
					//file not exists
				...
				success: function () {
					//file exists
					window.open("upload/Evaluacion_"+nroeval+".pdf");
...
{
	echo "<script type=''>
     swal('Datos Enviado Exitosamente!','','success'); 
     </script>"; 
}
