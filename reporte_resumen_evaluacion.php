...
WITH a AS (
		...
		SELECT e.nroeval,
			   e.codusu,
			   count(p.idpregunta) cantpregunta
		FROM evalucionparticipante e, pregunta p
		WHERE e.nroeval = p.nroeval
		group by e.nroeval,
			     e.codusu
        ...
        SELECT e.nroeval,
           ...
           round(sum(CASE
  				   ...
  				   ELSE
  		 				0
  			   END), 0) puntaje,
			   count(distinct r.codusu || r.item || r.nroeval) determinacion
		...
		SELECT r.nroeval,
			   r.codusu,
		       r.fechaenviado,
           	   sum(r.puntaje) puntaje,
			   count(r.idpregunta) determinacion
		FROM respuestafinal r
		group by r.nroeval,
			     r.codusu,
		 		 r.fechaenviado
		...
		SELECT r.nroeval,
		       ...
		       r.fechaenviado,
           	   sum(r.puntaje) puntaje,
			   count(r.codnumero) determinacion
		FROM respuestaleishmania r
		group by r.nroeval,
			     r.codusu,
		 		 r.fechaenviado
     )
