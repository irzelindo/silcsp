...
$mensage=0;
                 ...
                 $sql = 'select * from  motivorechazo order by codrechazo ';
                 ...
                 if ($numeroRegistros<=0)
                 ...
                 else
                    {
                   	if ($tiporeporte==1) // reporte HTML
             	    {
                        ...
                        echo '<p style="margin-left:0px"><strong>&nbsp;REPORTE DE TABLA DE MOTIVO DE RECHAZO</strong></p></td>';
    	                echo '</tr></table>';
                        ...
                        while ($row = pg_fetch_array($res))
    	                      {
     		                  ...
     		                  print '<tr>'
                                   .'<td class="tr" width="80px">'.$row['codrechazo'] .'</td>'
    		                       .'<td class="tr" width="300px">'.$row['nomrechazo'] .'</td>'
                                   .'</tr>';
    	                       }
    	                 print '</table><br />';
                         ...
                         $accion="Reporte Tabla Motivo Rechazo ";
					 ...
					 else // EXCEL O PDF
                     {
                          ...
                          print  '<table border=1>'
    	                        .'<tr><td style="font-weight:bold;font-size:1em;background-color:#F1F1F1;color:black;" colspan="2">REPORTE DE TABLA DE MOTIVO RECHAZO</td></tr>';
                          print '</table>';		
                        ...
                        while ($row = pg_fetch_array($res))
    	                      {
     		                  ...
     		                  print '<tr>'
                                   .'<td>'.$row['codrechazo'].'</td>'
    		                       .'<td>'.acentos($row['nomrechazo']).'</td>'
    		                       .'</tr>';
    	                       }
    	                 print '</table>';
                         ...
                         $fechaxx=date("Y-n-j", time());
                         $hora=date("G:i:s",time());
                         $accion="Reporte Tabla Motivo Rechazo XLS ";
                         $terminal = $_SERVER['REMOTE_ADDR'];
                         $a=archdlog($codusu,$codopcx,$fechaxx,$hora,$accion,$terminal);
                         // Fin grabacion de registro de auditoria	
                     }	// FIN EXCCEL
