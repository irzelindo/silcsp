...
if($indica == 1)
{
	$volver = "window.location.href='pacientes_turno.php?indica=$indica';";
}
else
{
	$volver = "window.location.href='pacientes.php';";
}
...
function ValidarFormulario($form)
{
	...
	$sql0 = "select * from paciente where cedula = '$cedula'";

	$res0=pg_query($con,$sql0);
	$countlc0=pg_num_rows($res0);
	...
	if($mensaje == "")
	{
		...
		if($cedula == "" or $tdocumento == '5')
		{
			...
			$cedula  = substr($pnombre,0,1).substr($snombre,0,1).substr($papellido,0,1).substr($sapellido,0,1).$dia.$mes.$anio;
		}
		...
		if($countlc0 == 0)
		{
					...
					$sqlnro = "select nextval('seq_paciente')";

					$resnro    = pg_query( $con, $sqlnro );
					$rownro 	 = pg_fetch_assoc($resnro);

					$nropaciente 	= $rownro["nextval"];
					...
					pg_query($con,"INSERT INTO paciente(
								nropaciente, fechareg, tdocumento, cedula, pnombre, snombre,
								papellido, sapellido, sexo, fechanac, edada, edadm, ecivil, nacionalidad,
								telefono, email, codexterno, estado, dccionr, paisr, coddptor,
								coddistr, nomyapefam, telefonof, celularf, obs, fechauact, codusup, tb, esetnia, codetnia)
				VALUES ('$nropaciente', '$fechareg', '$tdocumento', '$cedula', '$pnombre', '$snombre',
								'$papellido', '$sapellido', '$sexo', $fechanac, '$edada', '$edadm', '$ecivil', '$nacionalidad',
								'$telefono', '$email', '$codexterno', '$estado', '$dccionr', '$paisr', '$coddptor',
								'$coddistr', '$nomyapefam', '$telefonof', '$celularf', '$obs', '$fechauact', '$codusup', '$tb', '$esetnia', '$codetnia')");
					...
					if($indica == 1)
					{
						$respuesta->script("enviarpaciente("."'".$nropaciente."', '".$nomdocumento."', '".$cedula."', '".$pnombre."', '".$snombre."', '".$papellido."', '".$sapellido."')");
					}
					else
					{
						//$respuesta->script('swal("Datos", "Datos Registados Exitosamente!", "success")');

						$respuesta->redirect("modifica_pacientes.php?nropaciente=$nropaciente&mensage=2");
					}
					...
					$accion="Pacientes: Inserta-Reg.: Nro. Paciente: ".$nropaciente;
		...
		{
			$respuesta->script('swal("El paciente ya fue registrado !", "", "warning")');
		}
		...
		if($countsaa == 0)
		{
			...
			pg_query($consaa,"INSERT INTO public.paciente(
            cedula, coddpto, coddist, tdocumento, pnombre,
            snombre, papellido, sapellido, fechanac, sexo, ecivil, fultact, esetnia, codetnia)
    VALUES ('$cedula', '$coddptor', '$coddistr', '$tdocumento', '$pnombre', '$snombre', '$papellido', '$sapellido', $fechanac, '$sexo', '$ecivil', '$fechauact', '$esetnia', '$codetnia')");
		}
...
function enviarpaciente(valornropaciente,valortdocumento,valorcedula,valorpnombre,valorsnombre,valorpapellido,valorsapellido)
{

	var   doc = window.opener.document,
          theForm = doc.getElementById("formu"),
          cedula = doc.getElementById("cedula"),
          nropaciente = doc.getElementById("npaciente"),
          tdocumento = doc.getElementById("tdocumento"),
          pnombre = doc.getElementById("pnombre"),
          snombre = doc.getElementById("snombre"),
          papellido = doc.getElementById("papellido"),
          sapellido = doc.getElementById("sapellido");

        cedula.value = valorcedula;
		nropaciente.value = valornropaciente;
		tdocumento.value = valortdocumento;
		pnombre.value = valorpnombre;
		snombre.value = valorsnombre;
		papellido.value = valorpapellido;
		sapellido.value = valorsapellido;
...
<input type="hidden" id="indica" name="indica" value="<?php echo $indica; ?>">
...
<input type="text" name="tdocumento" id="tdocumento" list="listatipodocumento"  onkeypress="return validarcar(event)" value="1. Cedula">
...
<input type="radio" id="sexo1" name="sexo" value="1"><label for="sexo1"></label>
</div>
</td>
...
<input type="radio" id="sexo2" name="sexo" value="2"><label for="sexo2"></label>
</div>
</td>
...
<input type="text" name="ecivil" id="ecivil" list="listaestadocivil" value="<?php echo $ecivil; ?>" onkeypress="return validarcar(event)">
...
<input type="text" id="nacionalidad" name="nacionalidad" value="" maxlength="100" spellcheck="false" onkeypress="return validarcar(event)" onchange="conMayusculas(this)">
...
<input type="radio" id="estado1" name="estado" value="1" checked><label for="estado1"></label>
</div>
</td>
...
<input type="radio" id="estado2" name="estado" value="2"><label for="estado2"></label>
</div>
</td>
...
<hr id="empresas_detallesLine15">
<div class="sin-json">
	<input type="text" name="codetnia" id="codetnia" list="listaetnia" style="width: 80%; background-color:rgb(204, 204, 204);"  onkeypress="return validarcar(event)" onChange="ListaEtnia()" disabled>
...
<input type="text" id="codusup" name="codusup" value="<?php echo $codusu; ?>" spellcheck="false" onkeypress="return validarcar(event)">
...
<hr id="pacientes_detallesLine13">
<input type="text" name="tb" id="tb" list="listatb" value="<?php echo $tb; ?>" onkeypress="return validarcar(event)" style="font-size:13px;">
<hr id="pacientes_detallesLine19">
</div>
</div>
</div>
...
<div id="wb_LayoutGrid7">
	<div id="LayoutGrid7">
		<div class="row">
			<div class="col-1">
				<hr id="Line16"/>
				<button type="button" class="btn btn-primary btn-lg" onclick="xajax_ValidarFormulario(xajax.getFormValues(formu));">Guardar Datos</span></button>
				<hr id="Line11"/>
			</div>
			<div class="col-2">
			</div>
...
<datalist id="listatipodocumento">
  <option value="1. Cedula" selected>
