...
if($_GET['id'] != '')
{
	...
	$sql = "select * from apertura_cierre_caja where id = '$id'";
...
{
	$sql = "select * from apertura_cierre_caja where id = (select max(id) from apertura_cierre_caja where codusu = '$codusu' and tipo = '1')";
...
$sql1 = "select * from apertura_cierre_caja where id = (select max(id) from apertura_cierre_caja where codusu = '$codusu' and tipo = '1' and fechaapertura = '$fecha')";
...
function Confirmar( $form ) {
	...
	if ( $horacierre == "" ) {
		$mensaje .= '- Rellene el campo Hora Cierre!\n';
	...
	if ( $mensaje == "" ) 
	{
		...
		pg_query( $con, "UPDATE public.apertura_cierre_caja
							SET horacierre='$horacierre', tipo='2', codusuc = '$codusuaper'
							WHERE id = '$id'");
		
		$respuesta->script('swal("Apertura de Caja Realizada Exitosamente!", "", "success")');
		
		$respuesta->redirect("nuevo_arqueo_cierre_caja.php?codservicio=$codservicio1&id=$id");
		...
		<div id="pacientesLayoutGrid1">
			...
			<div id="wb_listas_ordenesLayoutGrid8">
				<div id="listas_ordenesLayoutGrid8">
					<div class="row">
						<div class="col-1">
							<div id="wb_listas_ordenesText12"> <span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>Hora Cierre:</strong></span> </div>
						</div>
						<div class="col-2">
							<hr id="listas_ordenesLine28">
							<input type="time" id="horacierre" name="horacierre" value="<?php echo $horacierre; ?>" spellcheck="false" readonly style="background-color: #DCDCDC;">
						</div>
					</div>
				</div>
			</div>
		  ...
		  <div class="col-1">
			  <?php
					if ( $v_153 == 2 || $v_153 == 3 ) 
					{
						if($countlc1 != 0)
						{
							echo '<button type="button" id="Buttonc" class="btn btn-primary btn-lg" onclick="xajax_Confirmar(xajax.getFormValues(formu));">Realizar Cierre</button>';
						}
					}
					?>
		  </div>
