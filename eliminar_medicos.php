...
@Header("Content-type: text/html; charset=utf-8");
   ...
   $sql1 = "select * from estrealizar where codmedico = '$id'";
   $res1=pg_query($con,$sql1);
   $numeroRegistros1=pg_num_rows($res1);

   $sql2 = "select * from ordtrabajo where codmedico = '$id'";
   ...
   if($numeroRegistros<=0)
      {
      $resultado1=pg_query($con,"DELETE FROM medicos WHERE codmedico = '$id'"); 
      ...
      if($resultado1==true)
           {
              ...
              $accion="Medicos: Elimina-Reg.: ".$id;
              $terminal = $_SERVER['REMOTE_ADDR'];
              $a=archdlog($codusu,$codopc,$fecha,$hora,$accion,$terminal);
              
              // Fin grabacion de registro de auditoria
              header("Location: medicos.php?mensage=1");	
           }
    	else
    	  {
      ...
      {		
      header("Location: medicos.php?mensage=4"); //no se puede borrar
      }
?>
