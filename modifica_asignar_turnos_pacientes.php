	...
	<script>
		$( function () {
			...
			jQuery( "#listapacientes" ).jqGrid( {
				url: 'datospacientesestudiosasignar.php?nroturno=<?php echo $nroturno; ?>&codservicio=<?php echo $codservicio; ?>',
				datatype: 'json',
				mtype: 'GET',
				loadonce: true,
				height: 200,
		...
		function confirmacion(code, cod, nro, est)
		{
			swal({
				...
				.then((willDelete) => {
				  if (willDelete)
				  {
					window.location = "eliminar_asignar_turnos_estudios.php?nordentra=" + code + "&nroestudio="+ cod + "&nroturno="+ nro + "&codservicio="+ est;
				  }
				  else
				  {
