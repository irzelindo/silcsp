	...
	<script>
		...
		$( function () {
			...
			jQuery( "#listapacientes" ).jqGrid( {
				url: 'datosnuevoenvio.php?codservicioe=<?php echo $codservicioe; ?>&codservicior=<?php echo $codservicior; ?>&desde=<?php echo $desde; ?>&hasta=<?php echo $hasta; ?>',
				datatype: 'json',
				mtype: 'GET',
				loadonce: true,
				height: 330,
				...
				colNames: [ 'Nro. Orden', 'Nombres y Apellidos', 'Tipo Muestra', 'Estado', 'Actual Establecimiento', '', '', '' ],
				colModel: [ {
						name: 'nordentra',
						index: 'nordentra',
						width: 120,
				...
				loadComplete: function (data) {
					...
					{
						var rowId = ids[i];
						var rowData = jQuery('#listapacientes').jqGrid ('getRowData', rowId);

						if (rowData.estado == 'Pendiente')
						{
							$("#jqg_listapacientes_" + rowId).css("visibility", "visible");
						}
						else
						{
							if (rowData.estado == 'Recibido')
							{
								$("#jqg_listapacientes_" + rowId).css("visibility", "visible");
							}
							else
							{
								$("#jqg_listapacientes_" + rowId).css("visibility", "hidden");
							}
						}
					}
				},
		...
		$( function () {
			...
			jQuery( "#listacodigobarra" ).jqGrid( {
				url: 'datosnuevoenvio1.php?codservicioe=9999',
				datatype: 'json',
				mtype: 'GET',
				loadonce: true,
				height: 330,
				...
				colNames: ['Borrar', 'Nro. Orden', 'Nombres y Apellidos', 'Tipo Muestra', 'Estado', 'Actual Establecimiento', '', '', '' ],
				colModel: [
				...
				loadComplete: function (data) {
					...
					{
						var rowId = ids[i];
						var rowData = jQuery('#listacodigobarra').jqGrid ('getRowData', rowId);

						if (rowData.estado == 'Pendiente')
						{
							$("#jqg_listacodigobarra_" + rowId).css("visibility", "visible");
						}
						else
						{
							if (rowData.estado == 'Recibido')
							...
							{
								$("#jqg_listacodigobarra_" + rowId).css("visibility", "hidden");
							}
						}
					}
				},
		...
		function gridReload() {
			...
			var codservicioe = jQuery( "#Combobox1" ).val();
			var codservicior = jQuery( "#Combobox3" ).val();
			var codcourier   = jQuery( "#Combobox2" ).val();
			...
			if(tiposeleccion == 1)
			{
					...
					jQuery( "#listapacientes" ).jqGrid( 'setGridParam', {
						url: "datosnuevoenvio.php?codservicioe=" + codservicioe + "&codservicior=" + codservicior + "&codcourier=" + codcourier + "&desde=" + desde + "&hasta=" + hasta,
						datatype: 'json'
					} ).trigger( "reloadGrid" );
		...
		function gridAgregar() {
			...
			jQuery.ajax({
						...
						type:'POST',
						dataType:'json',
						data:{codservicioe:codservicioe, codservicior:codservicior, codcourier:codcourier, hasta:hasta, desde:desde, qr:qr}
					}).done(function(respuesta){

							var datarow = {borrar:respuesta.borrar,nordentra:respuesta.nordentra,nomyape:respuesta.nomyape,nomtmuestra:respuesta.nomtmuestra,estado:respuesta.nomestado,nomservicio:respuesta.nomservicio,nropaciente:respuesta.nropaciente,estable:respuesta.codservicio,nromuestra1:respuesta.nromuestra1};
		...
		function GenerarAgrupamiento()
		{
			...
			if(tiposeleccion == 1)
			{
					var s = jQuery("#listapacientes").jqGrid('getGridParam','selarrrow'), selnromuestra = [],selnropaciente = [],selnordentra = [], selesta = [];

					var cantidad=s.length;
			}
			else
			{
					var s = jQuery("#listacodigobarra").getDataIDs(), selnromuestra = [],selnropaciente = [],selnordentra = [], selesta = [];
			...
			if ( codservicioe == "" )
			{
				mensaje = '- Rellene el campo Establecimiento que Envia!\n';

				jQuery("#Combobox1").css("background-color", "yellow");

			}
			else
			{
				jQuery("#Combobox1").css("background-color", "#DCDCDC");
			}

			if ( codservicior == "" )
			{
				mensaje += '- Rellene el campo Establecimiento que Recibe!\n';

				jQuery("#Combobox3").css("background-color", "yellow");

			}
			else
			{
				jQuery("#Combobox3").css("background-color", "white");
			}
			...
			else
			{
				if(cantidad != 0)
				{
					if(tiposeleccion == 1)
					{
							$.each(s,function()
							{
								selnromuestra.push(jQuery("#listapacientes").jqGrid('getCell',this,'nromuestra1'));
								selnropaciente.push(jQuery("#listapacientes").jqGrid('getCell',this,'nropaciente'));
								selnordentra.push(jQuery("#listapacientes").jqGrid('getCell',this,'nordentra'));

								selesta.push(jQuery("#listapacientes").jqGrid('getCell',this,'estable'));
					...
					else
					{
							$('#listacodigobarra tr').each(function() {
						       ...
						       var nordentra   = $(this).find("td").eq(1).html(),
									 		 nromuestra1 = $(this).find("td").eq(8).html(),
											 nropaciente = $(this).find("td").eq(6).html(),
											 estable     = $(this).find("td").eq(7).html();


									 if(nordentra != '')
									 {
											selnromuestra.push(nromuestra1);
											selnropaciente.push(nropaciente);
											selnordentra.push(nordentra);

											selesta.push(estable);
					...
					jQuery.ajax({
							url:'nuevo_agrupado.php',
							type:'POST',
							dataType:'json',
							data:{codservicioe:codservicioe, codservicior:codservicior, codcourier:codcourier, nromuestra:selnromuestra, nropaciente:selnropaciente, nordentra:selnordentra, grupo:grupo, codservicio:selesta, cantidad:cantidad}
						}).done(function(respuesta){
							...
							if(respuesta.grupo != 0)
							{
							   ...
							   jQuery("#Button1").attr('disabled');
								...
								swal({
										...
										window.open('impresion_envio.php?grupo='+respuesta.grupo, '_blank');
			...
			<div id="pacientesLayoutGrid1">
				<div class="row">
					<div class="col-1">
						<hr id="Line2">
						<div id="wb_Text2">
							<span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>Nro. Agrupamiento:  </strong></span>
						</div>
						<hr id="Line3">
					</div>
					...
					<div class="col-2">
						<hr id="Line4">
						<input type="text" id="grupo" name="grupo" value="<?php echo $grupo; ?>" readonly spellcheck="false">
						<hr id="Line5">
					</div>
				...
				<div class="row">
					<div class="col-1">
						<hr id="Line2">
						<div id="wb_Text2">
							<span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>Establecimiento que Env&iacute;a: </strong></span>
						</div>
						<hr id="Line3">
					</div>
						...
						<div class="selector-establecimiento">
							<select name="Combobox1" size="1" id="Combobox1" onChange="gridReload()" <?php if($codservicio1 !='' ){ echo 'disabled';} ?> style="<?php if($codservicio1 != ''){echo "background-color: #DCDCDC";}?>">
								...
								<?php
									...
									{
									   if($depto['codservicio'] == $codservicioe)
									   {
							...
							<input type="hidden" name="codservicioe" id="codservicioe" value="<?php echo $codservicioe; ?>">
						</div>
						<hr id="Line5">
					</div>
				...
				<div class="row">
					<div class="col-1">
						<hr id="Line2">
						<div id="wb_Text2">
							<span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>Establecimiento que Recibe: </strong></span>
						</div>
						<hr id="Line3">
					</div>
					...
					<div class="col-2">
						<hr id="Line4">
						<div class="selector-establecimiento">
							<select name="Combobox3" size="1" id="Combobox3" <?php if($codservicior1 !='' ){ echo 'disabled';} ?> style="<?php if($codservicior1 != ''){echo "background-color: #DCDCDC";}?>">
								<option value=""></option>
								<?php
									$tabla_dpto = pg_query($link, "select * from establecimientos order by codservicio");
									while($depto = pg_fetch_array($tabla_dpto))
									{
										 if($depto['codservicio'] == $codservicior)
									   {
										  echo "<option value = ".$depto['codservicio']." selected>".$depto['nomservicio']."</option>";
							...
							<input type="hidden" name="codservicior" id="codservicior" value="<?php echo $codservicior; ?>">
						</div>
						<hr id="Line5">
					</div>
				...
				<div class="row">
					<div class="col-1">
						<hr id="Line2">
						<div id="wb_Text2">
							<span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>Empresa Courier: </strong></span>
						</div>
						<hr id="Line3">
					</div>
					...
					<div class="col-2">
						...
						<div class="selector-establecimiento">
							<select name="Combobox2" size="1" id="Combobox2" onChange="gridReload()">
								<option value=""></option>
								<?php
									$tabla_dpto = pg_query($link, "select * from courier order by codcourier");
									while($depto = pg_fetch_array($tabla_dpto))
									{
									   echo "<option value = ".$depto['codcourier'].">".$depto['nomcourier']."</option>";
									}
				...
				<div class="row">
					<div id="page1Layer1_Container">
						...
						if ( $v_211 == 2 || $v_211 == 3 ) {
							...
							echo '<button type="button" class="btn btn-primary btn-lg" id="Button10" onclick="GenerarAgrupamiento()">Agrupar para envio</button>';
							//echo '<br><br>';
						...
						<div id = "mostrar1" style="display:hidden">
							<div class="jqGrid">
								<br/>
