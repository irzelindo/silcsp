...
function GenerarAgrupamiento()
{
	...
	$('#example').find("td.codnumero").each(function(index){
		...
		var nroeval        = $("#nroeval"+h).val(),
			...
			idpregunta     = $("#idpregunta"+h).val(),
			obs			   = $("#obs").val();
		
		
		
		jQuery.ajax({
						  url:'modifica_respuesta_parasito.php',
						  type:'POST',
						  dataType:'json',
						  data:{nroeval:nroeval, puntaje:puntaje, usuario:usuario, idpregunta:idpregunta, obs:obs}
			...
			<td><table width="700" border="0" align="center" cellspacing="0" cellpadding="3">
					  <thead style="display: table-header-group">
						...
						<tr style="font-size: 12px;">
						  ...
						  <td width="218" style="padding-top: 12px;">EVALUACION PARASITO INTESTINAL</td>
			  ...
			  <tr>
				<td>
					...
					<table width="700" border="0" align="center" cellspacing="0" cellpadding="3" id="example">
					  <thead>
						<tr>
							<td style="text-align: center; font-weight: bold; border-left: 3px solid; border-bottom: 3px solid; border-top: 3px solid;border-spacing: none;">Pregunta</td>
							<td style="text-align: center; font-weight: bold; border-bottom: 3px solid; border-top: 3px solid;border-spacing: none;">Repuesta Correcta</td>
							<td style="text-align: center; font-weight: bold; border-bottom: 3px solid; border-top: 3px solid;" >Respuesta Contestada</td>
					...
					<?php
						...
						while($row1=pg_fetch_array($q1) )
						{
								...
								echo '<tr>
										  <td>'.$row1['descripcio'].'</td>
										  <td style="text-align: center">'.$row1['correctar'].'</td>
										  <td style="text-align: center">'.$row1['respuestar'].'</td>
										  <td style="display:none;" class="codnumero"><input type="text" style="text-align: center;" value="'.$idpregunta.'" name="idpregunta'.$i.'" id="idpregunta'.$i.'" required></td>
