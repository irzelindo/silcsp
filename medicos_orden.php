...
$(function() {
    ...
    jQuery("#listamedicos").jqGrid({
        url:'datosmedicos_orden.php',
        datatype: 'json',
        mtype: 'GET',
    	loadonce:true,
        height: 320,
        ...
        colNames:['C&oacute;digo','Nombre','Ciudad'],
        colModel:[
				{name:'codmedico',index:'codmedico', width:100, align:"center", editable: true, searchoptions: {attr: {maxlength: 10,size: 7,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
                {name:'nomyape',index: "nomyape", width: 477, editable: true, searchoptions: {attr: {maxlength: 100,size: 80,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
                {name:'tipoprof',index: "tipoprof", width: 277, editable: true, searchoptions: {attr: {maxlength: 100,size: 80,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}}
                 ],

                caption: "LUGAR EXTRACCION",
                ignoreCase:true,
                pager: '#perpage',
                rowNum:7,
                rowList:[7,15,30],
	...
	grid = $("#listamedicos");
//	jQuery("#listamedicos").jqGrid('navGrid','#perpage',{edit:false,add:false,del:false,search:false},{},{},{},{closeAfterSearch:true, showQuery: true},{});
	jQuery("#listamedicos").jqGrid('setFrozenColumns');

    jQuery("#listamedicos").jqGrid('filterToolbar', {stringResult: true, searchOnEnter: false, defaultSearch : "cn"});
...
function enviarpaciente(valorcodmedico,valornomyape)
{

	var   doc = window.opener.document,
          theForm = doc.getElementById("formu"),
          codmedico = doc.getElementById("codmedico"),
          nomedico = doc.getElementById("nomedico");

        codmedico.value = valorcodmedico;
		nomedico.value = valornomyape;
	...
	swal({
			...
			.then((willDelete) => {
			  if (willDelete)
			  {
				...
				window.location = "eliminar_medicos.php?id=" + cod;
			  }
			  else
			  {
	...
	<div id="page1Layer1_Container">
		<br />
			...
			<?php
				if ($v_424==2 || $v_424==3)
				{
					 echo '<button type="button" class="btn btn-primary btn-lg" onclick="window.location.href=\'nuevo_medicos.php?indica='.$indica.'\';">Agregar</span></button>';
