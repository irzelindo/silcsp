...
$queryu = "SELECT * FROM resultados_utype u WHERE nro_orden = $1 ORDER BY CASE WHEN u.locus = 'HLA-DQB' THEN 99 ELSE 1 END, u.locus";
$stmu = pg_prepare($link,"stm",$queryu);
$resultu = pg_execute($link,"stm", array($nordentra));
$resultadosu = pg_num_rows($result);
							...
							<table width="886" border="0" align="center">
								  <thead style="display: table-header-group">
								    <tr style="border-bottom: solid 1px  ; border-top: solid 1px;">
										<td colspan="10" style="padding-bottom: 13px; padding-top: 13px;">
											<img src="images/logo-msp-labo.fw.png" style="width: 800px; align="center" height: 70px">
										</td>
									</tr>
									...
									<tr style="font-size: 12px;">
									  ...
									  <td colspan="4">'.$papellido.' '.$sapellido.' '.$pnombre.' '.$snombre.'</td>
									</tr>
			...
			while ($row3 = pg_fetch_array($result3))
			{
						...
						while ($row4 = pg_fetch_array($result4))
						{
							...
							while ($row7 = pg_fetch_array($result7))
							{
								...
								$nomdetermina   = ucfirst( $row7['nomdetermina'] );
								...
								//echo "realizado_por: $realizado_por - impreso: $impreso";

								if ( isset($realizado_por) && $impreso == "N" ) {
									echo '<tr><td>&nbsp;</td><td></td><td></td><td></td></tr>';
									echo '<tr>
												<td><strong>Realizado por:</strong></td>
												<td colspan="3">'.$realizado_por.'</td>
											</tr>';
									  echo '<tr>
												<td><strong>Confirmado por:</strong></td>
												<td colspan="3">'.$confirmado_por.'</td>
											</tr>
								...
								if($obs != '')
								{
									...
									echo '<tr>
									  <td style="vertical-align: top; padding: 8px;">'.ucfirst( $nomdetermina ).'<br><p style="font-size: 10px">Obs:'.$obs.'</p></td>
								...
								else
								{
									...
									if ( ucwords(strtolower($nomdetermina)) == "Observacion" ) {
										echo '<tr>
										  <td style="vertical-align: top; padding: 8px;" nowrap><strong>'.ucwords(strtolower($nomdetermina)).'</strong></td>
										  <td style="vertical-align: top; padding: 8px;" colspan="4">'.$resultado.'</td>
										</tr>
										';
									}else {
										echo '<tr>
								...
								$mostrarUsu="S";
								while ($row = pg_fetch_assoc($resultu)) {
								  ...
								  $tipificacionArray= explode(";",$tipificacion);
								  
								  if ( $mostrarUsu == "S") {
									  
									  echo '<tr>
												<td><strong>Locus:</strong></td>
												<td colspan="4"><strong>Tipicaci&oacute;n Final</strong></td>
											</tr>';
								  }
								  echo '<tr style="vertical-align: top; padding: 8px;">
									  <td>'.$locus.'</td>
									  <td colspan="4" style="padding: 3px;">'.$tipificacionArray[0].'<br>'.$tipificacionArray[1].'</td>
									</tr>';
								  $mostrarUsu="N";
								}
						...
						while ($rowvalidador = pg_fetch_array($resultvalidador))
						{
									...
									$archivo='firmas/'.$validador.'';

									echo '<div style="float: left;margin-right: 6%;">
													Validado por: '.$nomyape.' <br>Reg. Prof. Nro.: '.$nroregprof.'</div>';
						}
