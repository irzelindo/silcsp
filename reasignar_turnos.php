...
function ValidarFormulario($form) {
	...
	$sorden = "select * from ordtrabajo where nroturno = '$nroturno' and codservicio = '$codservicio1'";
    $rorden    = pg_query($con,$sorden);
    $cantorden = pg_num_rows($rorden);

	$sesta = "select * from estrealizar where nroturno = '$nroturno' and codservicio = '$codservicio1'";
    $resta    = pg_query($con,$sesta);
    $cantesta = pg_num_rows($resta);

	$query = "select * from turnos where nroturno = '$nroturno' and codservicio = '$codservicio1'";
	...
	if ( $mensaje == "" ) {
		...
		$dia  = substr($fechaturno,0,2);
		$mes  = substr($fechaturno,3,2);
		$anio = substr($fechaturno,6,4);
		...
		{
			$respuesta->script('swal("La Fecha elegida tiene que ser mayor o igual a la Fecha Actual!", "", "warning")');
		}
		else
		{
			if($cantferiado != 0)
			{
				$respuesta->script('swal("La Fecha elegida es un dia festivo: "'.$rowferiado["motivo"].'", "", "warning")');
			}
			else
			{
				...
				if($cantesta != 0)
				{
					pg_query($con, "UPDATE estrealizar
										SET nroturno='$nroturnonew', fecha='$fechaturno'
										WHERE nroturno = '$nroturno' and codservicio = '$codservicio1'" );
				}
				...
				pg_query($con, "INSERT INTO turnos(
						...
						obs, codturno)
				VALUES ('$nroturnonew', '$codservicio1', '$codarea1', '$nropaciente', '$fechaturno', '$horatur', '$urgente', '$asistio', '2', '$nroturreas', '2', '$obs', '$codturno1')" );

					if($indica == 1)
					{
						$respuesta->redirect("modifica_asignar_turnos_pacientes.php?nroturno=$nroturnonew&codservicio=$codservicio1");
					}
					else
					{
						$respuesta->redirect("modifica_turnos.php?nroturno=$nroturnonew&codservicio=$codservicio1");
					}
					...
					$accion = "Turno Reasignado: Inserta-Reg.: Nro. Turno: " . $codservicio1.$nroturno. "Nuevo Turno Nro. ".$codservicio1.$nroturnonew;
...
a(document).ready(function() {
	...
	a('#calendar').fullCalendar({
		...
		dayClick: function(date, jsEvent, view) {
			...
			if(jQuery("#codservicio").val() != '' && jQuery("#codarea").val() != '' && jQuery("#codturno").val() != '')
			{
				 a.ajax({
						url:'buscarturno.php',
						type:'POST',
						dataType:'json',
						data:{fecha:fecha, codservicio:jQuery("#codservicio").val(), codarea:jQuery("#codarea").val(), codturno:jQuery("#codturno").val()}
	...
	<div id="LayoutGrid3">
		<div class="row">
			<div class="col-1">
				<hr id="Line9"/>
				<div id="wb_Text1">
					<span style="color:#000000;font-family:Arial;font-size:13px;"><strong>USUARIO: </strong></span><span style="color:#FF0000;font-family:Arial;font-size:13px;"><strong><?php echo $elusuario;?></strong></span><span style="color:#000000;font-family:Arial;font-size:13px;"><strong><br><br>
					</strong></span><span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>REASIGNAR TURNOS</strong></span><span style="color:#000000;font-family:Arial;font-size:13px;"><strong><br></strong><br />
					</strong></span>
				</div>
			</div>
		</div>
...
<input type="hidden" id="fechaturno" name="fechaturno" value="">
<input type="hidden" name="nropaciente" id="nropaciente" value="<?php echo $nropaciente; ?>">
...
<div class="row">
	...
	<div class="col-md-12" style="width: 385px; float: left; padding-left: 15px">
	...
	<div class="col-2" style=" float: right; width: 624px">
	<?php
			 if ($v_121==2 || $v_121==3)
				{
				 //echo '<button type="button" class="btn btn-primary btn-lg" onclick="window.location.href=\'nuevo_sintomas.php\';">Agregar</span></button>';

				 echo '<input type="button" id="Button1" name="agregar" value="Agregar" onclick="xajax_ValidarFormulario(xajax.getFormValues(formu));">';
