...
// Se crea la conexión a la base de datos
   ...
   $v_151   = $_SESSION['V_151'];	// Ingresos y Cajas
	...
	while ($row = pg_fetch_array($res1))
	{
		...
		if($v_151==3)
	   {
		$respuesta->rows[$i]['cell'] = array($elimina,$row[norden],$row[codarancel],$row[nomarancel],$row[monto]); 	   	
	   }	
    else
	   { 
	   	$respuesta->rows[$i]['cell'] = array('S/Permiso',$row[norden],$row[codarancel],$row[nomarancel],$row[monto]);
	   } 
