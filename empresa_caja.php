...
$(function() {
    ...
    jQuery("#listaempresas").jqGrid({
        url:'datosempresascaja.php',
        datatype: 'json',
        mtype: 'GET',
    	loadonce:true,
        height: 320,
        ...
        colNames:['Revisar','Borrar','C&oacute;digo','Raz&oacute;n Social','RUC','Responsable','Tel&eacute;fono/Celular'],
        colModel:[
                ...
                {name:'codempresa',index: "codempresa", width: 100, editable: true, searchoptions: {attr: {maxlength: 10,size: 50,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
                {name:'razonsocial',index:'razonsocial', width:250, align:"left", editable: true, searchoptions: {attr: {maxlength: 200,size: 100,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
                {name:'ruc',index: "ruc", width: 100, editable: true, searchoptions: {attr: {maxlength: 50,size: 50,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
                {name:'responsable',index: "responsable", width: 250, editable: true, searchoptions: {attr: {maxlength: 150,size: 100,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
                {name:'telefono',index: "telefono", width: 150, editable: true, searchoptions: {attr: {maxlength: 60,size: 60,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}}                
                 ],
	...
	grid = $("#listaempresas");							
//	jQuery("#listaempresas").jqGrid('navGrid','#perpage',{edit:false,add:false,del:false,search:false},{},{},{},{closeAfterSearch:true, showQuery: true},{});
	jQuery("#listaempresas").jqGrid('setFrozenColumns');

    jQuery("#listaempresas").jqGrid('filterToolbar', {stringResult: true, searchOnEnter: false, defaultSearch : "cn"});
...
function enviarempresa(valorcodempresa,valorrazonsocial) 
{
	
	var   doc = window.opener.document,
          theForm    = doc.getElementById("formu"),
          npaciente  = doc.getElementById("npaciente"),
          nomyape    = doc.getElementById("nomyape");
	
        npaciente.value = valorcodempresa;
		nomyape.value   = valorrazonsocial;
...
{ 
	swal({
			...
			.then((willDelete) => {
			  if (willDelete) 
			  { 	     
				window.location = "eliminar_empresas.php?id=" + cod + "&coddist=" + est;
			  } 
			  else 
			  {
...
<div id="page1Layer1">
	<div id="page1Layer1_Container">
	  <?php
         if ($v_441==2 || $v_441==3) 
            {
    	     //echo '<button type="button" class="btn btn-primary btn-lg" onclick="window.location.href=\'nuevo_empresas.php\';">Agregar</span></button>';
             
             echo '<button type="button" id="Buttonc" class="btn btn-primary btn-lg" onclick="window.location.href=\'nuevo_empresas_caja.php\';">Agregar</button>';

            }
      ?>
      <div class="jqGrid">
        <br/>
        <table id="listaempresas"></table>
        <div id="perpage"></div>
      </div>        
        <br />
