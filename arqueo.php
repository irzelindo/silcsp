...
$sql = "select * from apertura_cierre_caja where id = (select max(id) from apertura_cierre_caja where codusu = '$codusu' and tipo = '2' and fechaapertura = '$fecha')";
...
x(document).ready(function() {
	x("#Buttonc").click(function(){
		...
		{
			window.location = "nuevo_arqueo.php?codservicio="+jQuery("#Combobox1").val();
		}
		else
		{
...
$(function() {
    ...
    jQuery("#listapacientes").jqGrid({
        url:'datosarqueo.php?codservicio=<?php echo $codservicio; ?>',
        datatype: 'json',
        mtype: 'GET',
    	loadonce:true,
        height: 200,
        ...
        colNames:['Ver', 'Revisar','Borrar','Fecha','Hora','Establecimiento', 'Monto'],
        colModel:[
				...
				{name:'hora',index: "hora", width: 110, align:"center", editable: true, searchoptions: {attr: {maxlength: 100,size: 80,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
			
                {name:'nomservicio',index: "nomservicio", width: 430, editable: true, searchoptions: {attr: {maxlength: 100,size: 80,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
...
{ 
	...
	jQuery("#listapacientes").jqGrid('setGridParam',{url:"datosarqueo.php?codservicio="+codservicio+"&desde="+desde+"&hasta="+hasta,datatype:'json'}).trigger("reloadGrid");
	
}
	
function confirmacion(fecha, hora) 
{ 
	swal({
			...
			.then((willDelete) => {
			  if (willDelete) 
			  {
				window.location = "eliminar_arqueo.php?fecha=" + fecha + "&hora=" + hora;
			  } 
			  else 
			  {
...
ARQUEO CAJA</strong></span><span style="color:#000000;font-family:Arial;font-size:13px;"><strong><br></strong><br />
					</strong></span>
				</div>
			</div>
		</div>
  ...
  <div id="page1Layer1_Container">
	  <?php
			 if ($v_154==1) 
			 {
				 if($countlc != 0)
				 {
					echo '<button type="button" id="Buttonc" class="btn btn-primary btn-lg">Agregar</span></button>';
				 }
