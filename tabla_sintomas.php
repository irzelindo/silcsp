...
$mensage=0;
                 ...
                 $sql = 'select * from sintomas order by codsintoma ';
                 ...
                 if ($numeroRegistros<=0)
                 ...
                 else
                    {
                   	if ($tiporeporte==1) // reporte HTML
             	    {
                        ...
                        echo '<p style="margin-left:0px"><strong>&nbsp;REPORTE DE TABLA DE SINTOMAS</strong></p></td>';
    	                echo '</tr></table>';
                        ...
                        while ($row = pg_fetch_array($res))
    	                      {
     		                  ...
     		                  print '<tr>'
                                   .'<td class="tr" width="80px">'.$row['codsintoma'] .'</td>'
    		                       .'<td class="tr" width="300px">'.$row['nomsintoma'] .'</td>'
					 ...
					 else // EXCEL O PDF
                     {
                          ...
                          print  '<table border=1>'
    	                        .'<tr><td style="font-weight:bold;font-size:1em;background-color:#F1F1F1;color:black;" colspan="3">REPORTE DE TABLA DE SINTOMAS</td></tr>';
                          print '</table>';		
                        ...
                        while ($row = pg_fetch_array($res))
    	                      {
     		                  ...
     		                  print '<tr>'
                                   .'<td>'.$row['codsintoma'].'</td>'
    		                       .'<td>'.acentos($row['nomsintoma']).'</td>'
    		                       .'<td>'.$row['tipo'].'</td>'
    		                       .'</tr>';
    	                       }
    	                 print '</table>';
                         ...
                         $fechaxx=date("Y-n-j", time());
                         $hora=date("G:i:s",time());
                         $accion="Reporte Tabla Sintomas XLS ";
                         $terminal = $_SERVER['REMOTE_ADDR'];
                         $a=archdlog($codusu,$codopcx,$fechaxx,$hora,$accion,$terminal);
                         // Fin grabacion de registro de auditoria	
                     }	// FIN EXCCEL
