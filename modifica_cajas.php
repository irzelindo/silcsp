...
$codcaja  = $_GET["id"]; //Identificador del registro
$codservicio  = $_GET["codservicio"]; //Identificador del registro

$query = "select * from cajas where codcaja = '$codcaja'  and codservicio='$codservicio'";
...
$accion="Accede a Cajas: ".$codcaja."-".$nomcaja;
...
#codservicio:focus, #region:focus, #codusuc:focus
{
   border-color: #66AFE9;
   -webkit-box-shadow: inset 0px 1px 1px rgba(0,0,0,0.075), 0px 0px 8px rgba(102,175,233,0.60);
   -moz-box-shadow: inset 0px 1px 1px rgba(0,0,0,0.075), 0px 0px 8px rgba(102,175,233,0.60);
...
function verificacajas()
{ 
     	if (window.document.formu.codcaja.value == "" || window.document.formu.nomcaja.value == ""  || window.document.formu.codservicio.value == ""  || window.document.formu.region.value == "") 
		    {
            if(window.document.formu.codcaja.value == "")
              {
    		    window.document.formu.codcaja.style.backgroundColor='yellow';    
              }
            else
              {
    		    window.document.formu.codcaja.style.backgroundColor='white';    
              } 

            if(window.document.formu.nomcaja.value == "")
              {
    		    window.document.formu.nomcaja.style.backgroundColor='yellow';    
              }
            else
              {
    		    window.document.formu.nomcaja.style.backgroundColor='white';    
              }
...
{
   var i;
   if (window.document.formu.codusuc.length > 0)
   {
	for (i = window.document.formu.codusuc.length - 1; i > -1; i--) 
	{ 
	window.document.formu.codusuc.remove(i);
	}  // aqui se me cae
   }
}
...
function ComponerLista2(codservicio) 
{
    //alert(codservicio+'primero');
		//window.document.f.region.disabled = true;
		window.document.formu.codusuc.lenght = 0;
		LimpiarCbo(window.document.formu.codusuc);
		//LimpiarCbo(window.document.f.establecimiento);
		SeleccionarUsuario(codservicio);
...
function SeleccionarUsuario(codservicio) 
{
    // alert(codservicio+'segundo');
	var o;
	window.document.formu.codusuc.disabled = true;
	<?php
		$tabla_codusuario = pg_query($link, "select * from usuarios order by nomyape");
		while($empl = pg_fetch_array($tabla_codusuario)) 
		{
	?>
	if( codservicio == '<?php echo $empl['codservicio']; ?>') 
	  {
	  o = window.document.createElement("OPTION");
	  o.text =  "<?php echo $empl['nomyape']; ?>";
	  o.value = "<?php echo $empl['codusu']; ?>";
	  window.document.formu.codusuc.options.add(o);
	  }
	<?php
		}
	?>
	window.document.formu.codusuc.disabled = false;
}
		...
		<div class="row">
			<div class="col-1">
				...
				<div id="wb_Text1">
					...
					<br />
					</strong></span><span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>ACTUALIZAR CAJAS</strong></span><span style="color:#000000;font-family:Arial;font-size:13px;"><br />
					<br />
					</span>
				</div>
			</div>
...
<form name="formu" method="post" action="actualiza_cajas.php">
    ...
    <div id="wb_LayoutGrid5">
    	<div id="LayoutGrid5">
    		<div class="row">
    			...
    			<div class="col-2">
    				<hr id="Line1"/>
    				<select name="codservicio" size="1" id="codservicio" onchange="Limpiar2()" onfocus="ComponerLista(window.document.formu.region.value)" style="background-color: #CCCCCC;" disabled="">
    					<option value=""></option>
                        <?php
                    ...
                    <input  type="hidden" name="codserviciox" id="codserviciox" value="<?php echo $codservicio;?>"/> 
    				<hr id="Line8"/>
    			</div>
    ...
    <div id="wb_LayoutGrid4">
    	<div id="LayoutGrid4">
    		<div class="row">
    			...
    			<div class="col-2">
    				<hr id="Line4"/>
                    <input type="text" class="form-control" placeholder="" style="background-color: #CCCCCC;" name="codcaja" id="codcaja" size="10" maxlength="10" readonly="" value="<?php echo $codcaja; ?>" onkeypress="return validarcar(event)" spellcheck="false"/>
    ...
    <div id="wb_LayoutGrid6">
    	<div id="LayoutGrid6">
    		<div class="row">
    			...
    			<div class="col-2">
    				<hr id="Line14"/>
                    <input type="text" class="form-control" placeholder="" id="nomcaja" name="nomcaja" size="100" maxlength="100" value="<?php echo $nomcaja;?>" onkeypress="return validarcar(event)" onchange="conMayusculas(this)" spellcheck="false"/>
                    
                    <input type="hidden" id="nomcajax" name="nomcajax" value="<?php echo $nomcaja;?>"/>
    	...
    	<div id="LayoutGrid5">
    		<div class="row">
    			<div class="col-1">
    				<hr id="Line6"/>
    				<div id="wb_Text3">
    					<span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>Usuario Asignado: </strong></span>
    				</div>
    			</div>
                
    			<div class="col-2">
    				<hr id="Line1"/>
    				<select name="codusuc" size="1" id="codusuc" onfocus="ComponerLista2(window.document.formu.codserviciox.value)" >
    					<option value=""></option>
                        <?php
            			$tabla_dpto = pg_query($link, "select * from usuarios order by nomyape");
            			while($depto = pg_fetch_array($tabla_dpto)) 
            			{
            		       if($depto["codusu"]==$codusuc)
                             {//Para que ponga predeterminado el que hay guardado
                             ?>
            		          <option value = "<?php echo $depto['codusu']; ?>" selected="" > 
            	              <?php echo $depto['nomyape']; ?></option>
                              <?php 
            			  	  }
                              else
            				  {?>
            		          <option value = "<?php echo $depto['codusu']; ?>" > 
            		          <?php echo $depto['nomyape']; ?></option>
                              <?php
            				  }?> 
                        <?php
                    ...
                    <input  type="hidden" name="codusucx" id="codusucx" value="<?php echo $codusuc;?>"/> 
    				<hr id="Line8"/>
    			</div>
		...
		<div class="row">
			<div class="col-1">
				...
				<?php
	             if ($v_443==2 || $v_443==3) 
	                {
				     echo '<input type="button" id="Button1" onclick="verificacajas()" name="guardar" value="Guardar Datos"/>';
	                }
	             ?>                
				<hr id="Line11"/>
			</div>
		...
		<div class="row">
			<div class="col-1">
				...
				<div id="wb_sintomas_detallesText1">
					<span style="color:#FF0000;font-family:Arial;font-size:13px;">[&nbsp;<a href="#" onclick="window.location.href='cajas.php';"> VOLVER </a>&nbsp;]</span>
