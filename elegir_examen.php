...
$v_171   = $_SESSION['V_171'];	// Evaluacion
...
function AgregarPregunta($form)
{
	...
	$query9 = "select * from evalucionparticipante where nroeval = '$nroeval1' and codusu = '$usuario1'";
	...
	if ($usuario == "") {
		$mensaje .= '- Rellene el campo Participante!';

		$respuesta->Assign( "usuario", "style.backgroundColor", "yellow" );

	} else {
		$respuesta->Assign( "usuario", "style.backgroundColor", "" );
	}
	...
	if ($numeroRegistros == 0) 
	{
		if ($mensaje == "")
		{
			$unro = pg_query( $con, "select max(item) as ultimo from evalucionparticipante where nroeval = '$nroeval1'" );
			...
			pg_query( $con, "INSERT INTO evalucionparticipante(
					nroeval, item, codusu)
			VALUES ('$nroeval1', '$item', '$usuario1')" );


			$respuesta->script("TraerLista($nroeval1)");
	...
	{
		$respuesta->script('swal("El participante ya fue agregado !", "", "warning")');
	}
	...
	<script>
		$(document).ready(function() {
			$("#Button10").click(function(){
				...
				window.location = "nuevo_examen.php";
	...
	<script>
		$(function() {
			...
			jQuery("#listapacientes").jqGrid({
				url:'datosexamen.php?nroeval=<?php echo $nroeval; ?>',
				datatype: 'json',
				mtype: 'GET',
				loadonce:true,
				height: 330,
				...
				colNames:['Revisar','Borrar','Participantes','Email','Imprimir','Nro. Evaluaci&oacute;n', 'Tipo Examen', 'Sector', 'Mes', 'A&ntilde;o'],
				colModel:[
						{name:'modificar', width:70, resizable:false, align:"center", editable: false, editoptions:{maxlength:"50"}, search: false},

						{name:'borrar', width:70, resizable:false, align:"center", editable: false, editoptions:{maxlength:"50"}, search: false},
					
						{name:'participante', width:70, resizable:false, align:"center", editable: false, editoptions:{maxlength:"50"}, search: false},
					
					    {name:'email', width:70, resizable:false, align:"center", editable: false, editoptions:{maxlength:"50"}, search: false},
					
					    {name:'imprimir', width:70, resizable:false, align:"center", editable: false, editoptions:{maxlength:"50"}, search: false},
						...
						{name:'razonsocial',index: "razonsocial", width: 250, editable: true, searchoptions: {attr: {maxlength: 100,size: 80,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
		...
		function gridReload() {
			...
			jQuery( "#listapacientes" ).jqGrid( 'setGridParam', {
				url: "datosexamen.php?nroeval=" + nroeval + "&mes=" + mes + "&anio=" + anio,
				datatype: 'json'
			} ).trigger( "reloadGrid" );
		}
	...
	<script language="JavaScript">
		...
		{ 
			swal({
					...
					.then((willDelete) => {
					  if (willDelete) 
					  {
						window.location = "eliminar_evaluacion.php?nroeval=" + cod;
					  } 
					  else 
					  {
		...
		function ListaEntidad()
		{
			...
			console.log(data, $("#listaentidad option").val());
				...
				$('#usuario1').val($('#listaentidad [value="' + value + '"]').data('value'));
			});

		}
		
		function TraerLista(nroeval)
		{
			$.ajax({
			   type: 'POST',
			   url: 'buscarparticipante.php',
			   data:{ nroeval: nroeval},
		...
		function enviar()
		{
			var nroeval 	= jQuery( "#nroeval1" ).val();
			var txtMensaje 	= jQuery( "#txtMensaje" ).val();

			jQuery.ajax({
                      url:'elegir_examen_email.php',
                      type:'POST',
                      dataType:'json',
                      data:{nroeval:nroeval, txtMensaje:txtMensaje},
					  success: function(data) {
		...
		function eliminar(nroeval, item, codusu) 
		{
			...
			jQuery.ajax({
					...
					dataType:'json',
					data:{nroeval:nroeval, item:item, codusu:codusu},
					success: function(data){
					
						TraerLista(data.nroeval);
					}

			});
		}
		
		function imprimir(nroeval, tipo)
		{
			if(tipo == 1)
			{
				window.location = 'imprimir_evaluacion.php?nroeval='+nroeval;
			}
			else
			{
				window.location = 'imprimir_eval.php?nroeval='+nroeval;
			}
...
<body onLoad="gridReload();">
		...
		<div id="wb_pacientesLayoutGrid1">
			<div id="pacientesLayoutGrid1">
				<div class="row">
					...
					<div class="col-2">
						<hr id="Line4">
						<input type="text" id="nroeval" name="nroeval" value="<?php echo $nroeval; ?>" spellcheck="false">
						<hr id="Line5">
					</div>
				...
				<div class="row">
					<div id="page1Layer1_Container">
						...
						if ( $v_171 == 2 || $v_171 == 3 ) {
							...
							echo '<button type="button" class="btn btn-primary btn-lg"  id="Button10">
					  			Agregar</button>';
	  ...
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  ...
		  <div class="modal-body">
			...
			<div class="row" style="text-align: left;padding-left: 47px;padding-bottom: 10px;padding-top: 10px;">Participantes</div>

			 <div class="row" style="padding-left: 45px;text-align: left;"><input type="text" name="usuario" id="usuario" list="listaentidad" style="width: 80%" onChange="ListaEntidad()" autofocus>
			 <input type="hidden" name="usuario1" id="usuario1" value="<?php echo $usuario; ?>" ></div>
	...
	<datalist id="listaentidad">
	  <?php
			$tabla_dpto = pg_query($link, "select * from usuarios");
			while($depto = pg_fetch_array($tabla_dpto))
			{
			   if($depto['codusu'] == $usuario)
			   {
				   echo '<option data-value = "'.$depto['codusu'].'" value = "'.$depto['codusu']."- ".$depto['nomyape'].'" selected></option>';

			   }
			   else
			   {
				   echo '<option data-value = "'.$depto['codusu'].'" value = "'.$depto['codusu']."- ".$depto['nomyape'].'"></option>';
			   }
			}
	  ...
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  ...
		  <div class="modal-body">
			...
			<div class="row"><input type="hidden" name="nroeval1" id="nroeval1">

			 </div> 
			  
			<div class="row" style="text-align: left;padding-left: 47px;padding-bottom: 10px;padding-top: 10px;">Mensaje</div>

			 <div class="row" style="padding-left: 45px;text-align: left;"><textarea name="txtMensaje" id="txtMensaje" rows="10" cols="60"></textarea>
			 </div>
