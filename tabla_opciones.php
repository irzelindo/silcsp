                 ...
                 if ($numeroRegistros<=0)
                 ...
                 else
                    {
                   	if ($tiporeporte==1) // reporte HTML
             	    {
                        ...
                        echo '<p style="margin-left:0px"><strong>&nbsp;REPORTE DE TABLA DE OPCIONES DEL SISTEMA</strong></p></td>';
    	                echo '</tr></table>';
                        ...
                        while ($row = pg_fetch_array($res))
    	                      {
     		                  ...
     		                  print '<tr>'
    		                       ...
    		                       .'<td class="tr" width="300px">'.$row['nomopc'] .'</td>'
    		                       .'<td class="tr" width="80px">'.$row['tipo'] .'</td>'
                                   .'<td class="tr" width="300px">'.$row['ayuda'] .'</td>'
                                   .'</tr>';
    	                       }
    	                 print '</table><br />';
					 ...
					 else // EXCEL O PDF
                     {
                          ...
                          print  '<table border=1>'
    	                        .'<tr><td style="font-weight:bold;font-size:1em;background-color:#F1F1F1;color:black;" colspan="4">REPORTE DE TABLA DE OPCIONES DEL SISTEMA</td></tr>';
                          print '</table>';		
    	
                          print  '<table border=1>';	
                          print  '<tr style="background-color:#F1F1F1;color:black;">'	
                                .'<td style="font-size:1em; width:80px;font-weight: bold;">C&oacute;digo</td>'
    	                        .'<td style="font-size:1em; width:200px;font-weight: bold;">Denominaci&oacute;n</td>'
                                .'<td style="font-size:1em; width:80px;font-weight: bold;">Tipo</td>'
                                .'<td style="font-size:1em; width:200px;font-weight: bold;">Ayuda</td>'
                                .'</tr>';
                        while ($row = pg_fetch_array($res))
    	                      {
     		                  ...
     		                  print '<tr>'
                                   .'<td>'.$row['codopc'].'</td>'
    		                       .'<td>'.acentos($row['nomopc']).'</td>'
    		                       .'<td>'.$row['tipo'].'</td>'
                                   .'<td>'.acentos($row['ayuda']).'</td>'
    		                       .'</tr>';
    	                       }
    	                 print '</table>';
                         ...
                         $fechaxx=date("Y-n-j", time());
                         $hora=date("G:i:s",time());
                         $accion="Reporte Tabla Opciones XLS ";
                         $terminal = $_SERVER['REMOTE_ADDR'];
                         $a=archdlog($codusu,$codopcx,$fechaxx,$hora,$accion,$terminal);
                         // Fin grabacion de registro de auditoria	
                     }	// FIN EXCCEL
