...
$(function() {
    ...
    jQuery("#listasectores").jqGrid({
        url:'datossectores.php',
        datatype: 'json',
        mtype: 'GET',
    	loadonce:true,
        height: 320,
        ...
        colNames:['Revisar','Borrar','C&oacute;digo','Nombre Sector'],
        colModel:[
                ...
                {name:'codsector',index:'codsector', width:100, align:"center", editable: true, searchoptions: {attr: {maxlength: 10,size: 7,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
                {name:'nomsector',index: "nomsector", width: 727, editable: true, searchoptions: {attr: {maxlength: 200,size: 80,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}}
                 ],
                                  
                caption: "SECTORES LABORATORIALES",
                ignoreCase:true,                
                pager: '#perpage',
                rowNum:7,
                rowList:[7,15,30],
	...
	grid = $("#listasectores");							
//	jQuery("#listasectores").jqGrid('navGrid','#perpage',{edit:false,add:false,del:false,search:false},{},{},{},{closeAfterSearch:true, showQuery: true},{});
	jQuery("#listasectores").jqGrid('setFrozenColumns');

    jQuery("#listasectores").jqGrid('filterToolbar', {stringResult: true, searchOnEnter: false, defaultSearch : "cn"});
...
{ 
	swal({
			...
			.then((willDelete) => {
			  if (willDelete) 
			  { 	     
				window.location = "eliminar_sectores.php?id=" + cod ;
			  } 
			  else 
			  {
      ...
      <div class="jqGrid">
    	...
    	<?php
         if ($v_4110==2 || $v_4110==3) 
            {
    	     //echo '<button type="button" class="btn btn-primary btn-lg" onclick="window.location.href=\'nuevo_sectores.php\';">Agregar</span></button>';
             
             echo '<input type="submit" id="Button1" onclick="window.location.href=\'nuevo_sectores.php\';" name="agregar" value="Agregar">';
