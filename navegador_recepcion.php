...
$volver = "window.location.href='recepcion.php'";
...
$xajax->register( XAJAX_FUNCTION, 'RechazaMuestra' );
...
function ValidarFormulario($form) {
	...
	if ( $codservder == "" ) {
		$mensaje = '- Rellene el campo Entidad Derivante!\n';

		$respuesta->Assign( "codservder", "style.backgroundColor", "yellow" );

	} else {
		$respuesta->Assign( "codservder", "style.backgroundColor", "white" );
	}
	...
	if ( $fecharec == "" ) {
		...
		$respuesta->Assign( "fecharec", "style.backgroundColor", "white" );
	}
	...
	if ( $horarec == "" ) {
		...
		$respuesta->Assign( "horarec", "style.backgroundColor", "white" );
	}
	...
	if ( $mensaje == "" ) {
		...
		pg_query( $con, "UPDATE datoagrupado
   							SET fecharec='$fecharec', obs='$obs', codservicior='$codservder1', estado='3'
		 					WHERE nordentra = '$nordentra' and codservicio = '$codservicio1' and nromuestra = '$nromuestra'" );
		...
		pg_query( $con, "UPDATE ordtrabajo
						 ...
						 WHERE nordentra = '$nordentra' and codservicio = '$codservicio1'" );
		
		pg_query( $con, "UPDATE estrealizar
   							SET codservact='$codservder1'
		 					WHERE nordentra = '$nordentra' and codservicio = '$codservicio1' and nromuestra = '$nromuestra'" );

		$respuesta->script('swal("Datos", "Recepcion Confirmada!", "success")');
		...
		$accion = "Recepcion de Envio: Nro. Orden: " . $codservicio1.$nordentra;
...
function RechazaMuestra($form) {
	...
	$sql = "select * from datoagrupado where nordentra = '$nordentra' and codservicio = '$codservicio1'";

	$res 	 = pg_query( $con, $sql );

	if ( $codrechazo == "") {
		$mensaje .= '- Rellene el campo Motivo!\n';

		$respuesta->Assign( "codrechazo", "style.backgroundColor", "yellow" );

	} else {
		$respuesta->Assign( "codrechazo", "style.backgroundColor", "white" );
	}


	if ( $mensaje == "" ) {
		$band = 0;
		while ($rowp = pg_fetch_array($res))
		{
			$grupo 		= $rowp["grupo"];
			$nromuestra = $rowp["nromuestra"];
			$estado 	= $rowp["estado"];
			
			$sql1 = "select * from rechazados where grupo = '$grupo' and nromuestra = '$nromuestra'";
		...
		grupo, nromuestra, fecharechazo, codrechazo, codusu, nropaciente, obs)
		VALUES ('$grupo', '$nromuestra', '$fecharec', '$codrechazo', '$codusu', '$npaciente', '$obs')" );
				}
				else
				{
					pg_query( $con, "UPDATE rechazados
								SET codrechazo='$codrechazo'
								WHERE grupo = '$grupo' and nromuestra = '$nromuestra'" );
				}
		...
		if($band == 1)
		{
			...
			$respuesta->script('swal("Datos", "Muestra Rezada!", "success")');
			...
			$fecha1 = date( "Y-n-j", time() );
			$hora = date( "G:i:s", time() );
			$accion = "Rechazo de Envio: Nro. Orden: " . $codservicio1.$nordentra;
			$terminal = $_SERVER[ 'REMOTE_ADDR' ];
			$a = archdlog( $_SESSION[ 'codusu' ], $codopc, $fecha1, $hora, $accion, $terminal );
			// Fin grabacion de registro de auditoria
		}
		else
		{
			if($band == 2)
			{
				$respuesta->script( 'swal("Atencion", "Recepcion ya fue Confirmada!", "warning")' );
			}
			else
			{
				$respuesta->script( 'swal("Atencion", "Muestra ya Rechazada!", "warning")' );
			}
		}
...
<head>
	...
	<script>
		$( function () {
			...
			jQuery( "#listapacientes" ).jqGrid( {
				url: 'datospacientesestudios1.php?nordentra=<?php echo $nordentra; ?>&codservicio=<?php echo $codservicio; ?>&nromuestra=<?php echo $nromuestra; ?>',
				datatype: 'json',
				mtype: 'GET',
				loadonce: true,
				height: 200,
				...
				colNames: ['C&oacute;digo Externo', 'C&oacute;digo', 'Estudios', 'Abreviatura'],
				colModel: [ {
					name: 'codexterno',
					index: 'codexterno',
					width: 120,
	...
	<script language="JavaScript">
		...
		{ 
			 ...
			 else
			 {
				...
				if (!permitida) 
				{
				 ...
				 {
					swal("Datos", "Se Adjunto correctamente!", "success");
		...
		function openWin() 
		{
    		nordentra   = window.document.formu.nordentra.value;
			...
			$.ajax({
				...
				error: function () {
					...
					swal("Atencion!", 'El archivo falta adjuntar!', "warning");
		...
		<div id="LayoutGrid1">
			<div class="row">
				<div class="col-1">
					...
					<div id="wb_Image4">
						<img src="images/banner1lcsp.png" id="Image4" alt=""/>nordentranordentranordentra
					</div>
				</div>
			</div>
		</div>
		...
		<div id="turnos_detallesLayoutGrid5">
			<div class="row">
				...
				<div class="col-2">
					...
					<input type="date" id="fecharec" name="fecharec" value="<?php echo $fecharec; ?>" autofocus>
					<hr id="turnos_detallesLine15">
			      <input type="hidden" name="nroturno" id="nroturno" value="<?php echo $nroturno; ?>">
					<input type="hidden" name="nromuestra" id="nromuestra" value="<?php echo $nromuestra; ?>">
				</div>
...
<div id="wb_turnos_detallesLayoutGrid5">
		<div id="turnos_detallesLayoutGrid5">
			<div class="row">
				...
				<div class="col-2">
					<hr id="turnos_detallesLine14">
					<input type="time" id="horarec" name="horarec" value="<?php echo $horarec; ?>" >
					<hr id="turnos_detallesLine15">
				</div>
			</div>
		</div>
		...
		<div id="courier_detallesLayoutGrid2">
			<div class="row">
				...
				<div class="col-2">
					...
					<select name="codservder" size="1" id="codservder">
                        <option value=""></option>
                        <?php
	...
	<div id="wb_pacientes_detallesLayoutGrid1">
		<div id="pacientes_detallesLayoutGrid1">
			<div class="row">
				...
				<div class="col-4">
					<input type="text" id="nordentra" name="nordentra" value="<?php echo $nordentra; ?>" readonly>
	  ...
	  <div id="ordenes_detallesLayoutGrid3">
		<div class="row">
		  <div class="col-1">
			...
			<div id="wb_ordenes_detallesText3"> <span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>Hora de salida prevista: <br>
			  </strong></span> </div>
		  </div>
		  <div class="col-2">
			<hr id="ordenes_detallesLine8">
			<input type="time" id="horasal" name="horasal" value="<?php echo $horasal; ?>" spellcheck="false">
	  ...
	  <div id="ordenes_detallesLayoutGrid4">
		<div class="row">
		  <div class="col-1">
			...
			<input type="button" id="ordenes_detallesButton1" onclick="buscadormedico();" name="medico" value="Buscar Medico">
	...
	<div id="pacientes_detallesLayoutGrid11">
		...
		<div class="row">
			<div class="col-1">
				...
				<input type="button" id="pacientes_detallesButton1" onclick="xajax_ValidarFormulario(xajax.getFormValues('formu'));" name="reasignar" value="Confirmar Recepci&oacute;n" >
...
<div id="wb_turnos_detallesLayoutGrid3">
	<div id="turnos_detallesLayoutGrid3">

		<div class="row">
			<div class="col-1">
				<hr id="turnos_detallesLine6">
				<input type="button" id="turnos_detallesButton2" name="impetiquetas" value="Rechaza Muestra" onClick="xajax_RechazaMuestra(xajax.getFormValues('formu'));">
				<hr id="turnos_detallesLine9">
			</div>
			<div class="col-2">
				<hr id="turnos_detallesLine8">
				<div id="wb_empresas_detallesText10">
					<span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>Motivo: </strong></span>
				</div>
				<hr id="turnos_detallesLine10">
			</div>
			<div class="col-3">
				<hr id="turnos_detallesLine7">
				<select name="codrechazo" size="1" id="codrechazo">
                       <option value=""></option>
                        <?php
							$tabla_dpto = pg_query($link, "select * from motivorechazo");
							while($depto = pg_fetch_array($tabla_dpto)) 
							{
							   echo '<option value="'.$depto["codrechazo"].'">'.$depto["nomrechazo"].'</option>';
							}
            			?>
    				</select>
			</div>
