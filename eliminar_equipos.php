...
@Header("Content-type: text/html; charset=utf-8");
   ...
   $sql1 = "select * from resultados where codequipo = '$id'";
   $res1=pg_query($con,$sql1);
   $numeroRegistros1=pg_num_rows($res1);

   $sql2 = "select * from resultadosmicro where codequipo = '$id'";
   $res2=pg_query($con,$sql2);
   $numeroRegistros2=pg_num_rows($res2);

   $sql3 = "select * from determinacionequipo where codequipo = '$id'";
   $res3=pg_query($con,$sql3);
   $numeroRegistros3=pg_num_rows($res3);

   $sql4 = "select * from resultadorepeticion where codequipo = '$id'";
   $res4=pg_query($con,$sql4);
   $numeroRegistros4=pg_num_rows($res4);

   $sql5 = "select * from resultadoequipo where codequipo = '$id'";
   ...
   if($numeroRegistros<=0)
      {
      $resultado1=pg_query($con,"DELETE FROM equipos WHERE codequipo = '$id'"); 
      ...
      if($resultado1==true)
           {
              ...
              $accion="Equipos Analizadores: Elimina-Reg.: ".$id;
              $terminal = $_SERVER['REMOTE_ADDR'];
              $a=archdlog($codusu,$codopc,$fecha,$hora,$accion,$terminal);
              
              // Fin grabacion de registro de auditoria
              header("Location: equipos.php?mensage=1");	
           }
    	else
    	  {
      ...
      {		
      header("Location: equipos.php?mensage=4"); //no se puede borrar
      }
?>
