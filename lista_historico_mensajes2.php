...
if ($tiporeporte==1)//Navegador
   ...
   print '<head><link rel="shortcut icon" href="images/icono.ico"/><link rel="stylesheet" type="text/css" href="images/style.css" />'
         .'<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=yes"/><title>Reporte - Historico de Mensajes</title><script type="text/javascript" src="reflection.js"></script></head><body style="color: black;background: white;font-size: 90%;">';	
   }
         ...
         if ($di2=="" && $me2=="" && $an2=="")  // SI FECHA HASTA ES BLANCO
            ...
            if ($ok1=="SI" && $ok2=="SI")
              ...
              if ($mensage==5)
                 ...
                 {
                 $sql = "select * from mensajes where codusu<>'' and ".$condicion." order by codusu,fecha,hora";
                 ...
                 if ($numeroRegistros<=0)
                    ...
                    echo '<p style="margin-left:0px"><strong>&nbsp;REPORTE DE HISTORICO DE MENSAJES</strong></p></td>';
	                echo '</tr></table>';
                    ...
                    print '<br><table class="cuadro">'
                          ...
                          .'<td class="hr">SMS</td>'
                          .'<td class="hr">Estado de SMS</td>'
                          .'<td class="hr">e-mail</td>'
                          .'<td class="hr">Estado de e-mail</td>'
                          .'<td class="hr">Alertas</td>'
		                  .'<td class="hr">Estado de Alertas</td></tr>';
                    ...
                    while ($row = pg_fetch_array($res))
                          ...
                          $smsx='';
                          $sms=$row['sms'];
                          if($sms==1){$smsx=' SI';}else{if($sms==2){$smsx=' NO';}}
                          $emailx='';
                          $email=$row['email'];
                          if($email==1){$emailx=' SI';}else{if($email==2){$emailx=' NO';}}
                          $alertax='';
                          $alerta=$row['alerta'];
                          if($alerta==1){$alertax=' SI';}else{if($alerta==2){$alertax=' NO';}}
                          $estadosmsx='';
                          $estadosms=$row['estadosms'];
                          if($estadosms==1){$estadosmsx=' ENVIADO';}else{if($estadosms==2){$estadosmsx=' PENDIENTE';}}
                          $estadoemailx='';
                          $estadoemail=$row['estadomail'];
                          if($estadoemail==1){$estadoemailx=' ENVIADO';}else{if($estadoemail==2){$estadoemailx=' PENDIENTE';}}
                          $estadoalertax='';
                          $estadoalerta=$row['estadoalerta'];
                          if($estadoalerta==1){$estadoalertax=' ENVIADO';}else{if($estadoalerta==2){$estadoalertax=' PENDIENTE';}}
 		                  print '<tr>'
                               .'<td class="tr" width="80px">'.$row['codusu'] .'</td>'
		                       .'<td class="tr" width="85px">'.$fcalc .'</td>'
		                       .'<td class="tr" width="70px">'.$row['hora'] .'</td>'
		                       .'<td class="tr" width="250px">'.$nomservicio .'</td>'
                               .'<td class="tr" width="70px">'.$smsx .'</td>'
                               .'<td class="tr" width="70px">'.$estadosmsx .'</td>'
                               .'<td class="tr" width="70px">'.$emailx .'</td>'
                               .'<td class="tr" width="70px">'.$estadoemailx .'</td>'
                               .'<td class="tr" width="70px">'.$alertax .'</td>'
		                       .'<td class="tr" width="70px">'.$estadoalertax .'</td>'
		                       .'</tr>';
	                       }
	                 print '</table><br />';
                     ...
                     $accion="Reporte Historico Mensajes: ".$subtit;
					 ...
					 else // EXCEL
					 {
                      ...
                      print  '<table border=1>'
	                        .'<tr><td style="font-weight:bold;font-size:1em;background-color:#F1F1F1;color:black;" colspan="10">REPORTE DE HISTORICO DE CONTRASE&Ntilde;A</td></tr>'
	                        .'<tr>'
	                        .'<td style="font-weight:bold;font-size:1em;background-color:#F1F1F1;color:black;" colspan="10"><b>Considerando:</b> '.$subtit.'<br /></td>'
	                        .'</tr>';
                      print '</table>';		
                      ...
                      print  '<tr style="background-color:#F1F1F1;color:black;">'	
                            ...
                            .'<td style="font-size:1em; width:2500px;font-weight: bold;">Establecimiento</td>'
                            .'<td style="font-size:1em; width:80px;font-weight: bold;">SMS</td>'
                            .'<td style="font-size:1em; width:80px;font-weight: bold;"> Estado SMS</td>'
                            .'<td style="font-size:1em; width:80px;font-weight: bold;">e-mail</td>'
                            .'<td style="font-size:1em; width:80px;font-weight: bold;"> Estado mail</td>'
                            .'<td style="font-size:1em; width:80px;font-weight: bold;">Alerta</td>'
                            .'<td style="font-size:1em; width:80px;font-weight: bold;"> Estado Alerta</td>'
                            .'</tr>';
                    while ($row = pg_fetch_array($res))
	                      {
                          $fu = $row["fecha"];
                     ...
                     $fechaxx=date("Y-n-j", time());
                     $hora=date("G:i:s",time());
                     $accion="Reporte Historico Mensajes XLS: ".$subtit;
                     $terminal = $_SERVER['REMOTE_ADDR'];
                     $a=archdlog($codusu,$codopcx,$fechaxx,$hora,$accion,$terminal);
                     // Fin grabacion de registro de auditoria					 	
					 }	
