...
// Se crea la conexión a la base de datos
   ...
   $v_151   = $_SESSION['V_151'];	// Ingresos y Cajas
	...
	while ($row = pg_fetch_array($res1))
	{
		...
		if($estado == 1)
		{
			$elimina = '<div id="wb_FontAwesomeIcon4"><a href="#" onclick="confirmacion('."'".$row[nroingreso]."', '".$row[nrorecibo]."', '".$row[nroreciboser]."', '".$row[norden]."'".');" style="text-decoration:none"><div id="FontAwesomeIcon4"><i class="fa fa-trash-o">&nbsp;</i></div></a></div>';
		}
		else
		{
			$elimina  = "";
		...
		if($v_151==3)
	   {
		$respuesta->rows[$i]['cell'] = array($elimina,$row[norden],$row[codarancel],$row[nomarancel],$row[cantidad],number_format($row[monto], 0, ",", "."),number_format($row[total], 0, ",", ".")); 	   	
	   }	
    else
	   {   
	   	$respuesta->rows[$i]['cell'] = array('S/Permiso',$row[norden],$row[codarancel],$row[nomarancel],$row[cantidad],number_format($row[monto], 0, ",", "."),number_format($row[total], 0, ",", "."));
	   } 
