...
if($_GET['fechaturno'] != "")
{
	$dia2=substr($_GET['fechaturno'],0,2);
	$mes2=substr($_GET['fechaturno'],3,2);
	$anho2=substr($_GET['fechaturno'],6,4);

	$fechaturno = $anho2.'-'.$mes2.'-'.$dia2;
}
else
{
	$dia2="";
...
$xajax->register( XAJAX_FUNCTION, 'RecuperarDatos' );
...
function ValidarFormulario($form)
{
	...
	$sql = "select * from turnos where nroturno = '$nrotur' and codservicio = '$codservicio1'";
	...
	if ( $mensaje == "" ) {
		...
		if ( $countlc == 0 )
		{
            ...
            obs, codturno)
    VALUES ('$nrotur', '$codservicio1', '$codarea1', '$npaciente', '$fechatur', '$horatur', '$urgente', '$asistio', '2', '$nroturreas', '2', '$obs', '$codturno1')" );
			...
			$respuesta->redirect("modifica_turnos.php?nroturno=$nrotur&codservicio=$codservicio1&mensage=2");
			...
			$accion = "Turno: Inserta-Reg.: Nro. Turno: " . $codservicio1.$nrotur;
			$terminal = $_SERVER[ 'REMOTE_ADDR' ];
			$a = archdlog( $_SESSION[ 'codusu' ], $codopc, $fecha1, $hora, $accion, $terminal );
			// Fin grabacion de registro de auditoria
		} else {
			pg_query( $con, "UPDATE turnos
   							SET codservicio='$codservicio1', codarea='$codarea1', nropaciente='$npaciente', fechatur='$fechatur',
       						horatur='$horatur', urgente='$urgente', asistio='$asistio', obs='$obs', codturno='$codturno1'
			...
			$fecha1 = date( "Y-n-j", time() );
			$hora = date( "G:i:s", time() );
			$accion = "Turnos: Modificar-Reg.: Nro. Turno: " . $codservicio1.$nrotur;
			$terminal = $_SERVER[ 'REMOTE_ADDR' ];
			$a = archdlog( $_SESSION[ 'codusu' ], $codopc, $fecha1, $hora, $accion, $terminal );
			// Fin grabacion de registro de auditoria
		}
...
function GenerarOrden($form) {
	...
	if ($countlc == 0)
	{
		$respuesta->script('swal("Debe Agregar al menos un Estudio, para Generar la Orden de Trabajo!", "", "warning")');
	}
	else
	{
		while ($row = pg_fetch_array($res))
		{
			...
			$sql1 = "select * from ordtrabajo where nroturno = '$nrotur'";
			...
			{
				$sqlnro = "select nextval('seq_ordtrabajo')";

				$resnro = pg_query( $con, $sqlnro );
				$rownro 	 = pg_fetch_assoc($resnro);

				$nordentra 	= $rownro["nextval"];
				...
				nordentra, codservicio, nropaciente, fecharec, horarec, nroturno, codusu, codservrem, coddgvs, laboratorio)
				VALUES ('$nordentra', '$codservicio1', '$npaciente', '$fecharec', '$horarec', '$nrotur', '$codusu', '$codservicio1', '0', '$laboratorio')" );
...
function NavegadorOrden($form) {
	...
	if($nordentra != '')
	{
		$respuesta->redirect("navegador_ordenes.php?nordentra=$nordentra&codservicio=$codservicio1&nroturno=$nrotur");
	}
	else
	{
...
function GenerarMuestra($form)
{
	...
	$querymuestra = "select  distinct e.codestudio,
			...
			and   er.codservicio= '$codservicio1'";

	$resultmuestra = pg_query( $con, $querymuestra );

	$query2 = "select * from ordtrabajo where nroturno = '$nrotur' and codservicio = '$codservicio1'";
	...
	while ( $rowmuestra = pg_fetch_array( $resultmuestra ) )
	{
		...
		else
		{
			...
			{
				$unro = pg_query( $con, "select COALESCE(max(nromuestra), '0') as ultimo from estrealizar where codservicio = '$codservicio1'" );
		...
		pg_query($con, "UPDATE estrealizar er
								...
								WHERE er.nroturno = '$nrotur' and er.codservicio = '$codservicio1' and codestudio = '$codestudio' and codtmuestra = '$codtmuestra' and er.nromuestra is null");
	}

	return $respuesta;
}

function RecuperarDatos($form) {
	extract($form);
	...
	$sql = "select * from paciente where nropaciente = '$npaciente'";
	...
	if ( $countlc != 0 )
	{
		...
		$respuesta->Assign( "tdocumento", "value", $nomdocumento);
		$respuesta->Assign( "cedula", "value", $cedula);
		$respuesta->Assign( "pnombre", "value", $pnombre);
		$respuesta->Assign( "snombre", "value", $snombre);
		$respuesta->Assign( "papellido", "value", $papellido);
		$respuesta->Assign( "sapellido", "value", $sapellido);

	}
	else
	{
		$respuesta->Assign( "nomdocumento", "value", "");
		$respuesta->Assign( "cedula", "value", "");
		$respuesta->Assign( "pnombre", "value", "");
		$respuesta->Assign( "snombre", "value", "");
		$respuesta->Assign( "papellido", "value", "");
		$respuesta->Assign( "sapellido", "value", "");

		$respuesta->script('swal("No existe Codigo de Paciente Introducida !", "", "warning")');
	}
	return $respuesta;
}
...
<head>
	...
	<script>
		$( function () {
			...
			jQuery( "#listapacientes" ).jqGrid( {
				url: 'datospacientesestudios.php?nroturno=-1',
				datatype: 'json',
				mtype: 'GET',
				loadonce: true,
				height: 200,
	...
	<script language="JavaScript">
			...
			swal({
				...
				.then((willDelete) => {
				  if (willDelete)
				  {
					...
					window.location = "eliminar_turnos_estudios.php?nordentra=" + code + "&nroestudio="+ cod + "&nroturno="+ nro + "&codservicio="+ est;
				  }
				  else
				  {
		...
		function gridReload()
		{
			...
			jQuery("#listapacientes").jqGrid('setGridParam',{url:"datospacientesestudios.php?nroturno="+nroturno+"&codservicio="+codservicio,datatype:'json'}).trigger("reloadGrid");
		...
		<div id="turnos_detallesLayoutGrid5">
			<div class="row">
				...
				<div class="col-2">
					...
					<input type="date" id="fechatur" name="fechatur" value="<?php echo $fechaturno; ?>" readonly spellcheck="false">
	...
	<div id="wb_pacientes_detallesLayoutGrid1">
		<div id="pacientes_detallesLayoutGrid1">
			<div class="row">
				...
				<div class="col-2">
				  <input type="text" name="npaciente" id="npaciente" value="<?php echo $nropaciente; ?>" autofocus onChange="xajax_RecuperarDatos(xajax.getFormValues(formu));">
				...
				<div class="col-2">
				  <hr id="turnos_detallesLine8">
					<?php
						if ($v_123==1)
						{
						 	$l = "window.open('impresion_indicacion.php?nroturno=".$nroturno."&codservicio=".$codservicio."')";

							echo '<input type="button" id="turnos_detallesButton5" onclick="'.$l.';return false;" name="hcli" value="Imprimir Indicaciones">';
						}
					?>
			...
			<div class="row">
				<div class="col-1">
					...
					<div id="wb_sintomas_detallesText1">
						<span style="color:#FF0000;font-family:Arial;font-size:13px;">[&nbsp;<a href="#" onclick="window.location.href='turnos.php'"> VOLVER </a>&nbsp;]</span>
