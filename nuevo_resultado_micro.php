...
function ValidarFormulario($form) {
	...
	if ( $mensaje == "" ) {
		...
		$unro = pg_query( $con, "select max(CAST(coalesce(nroresul, '0') AS integer)) as ultimo from resultadomicroorganismo where nordentra = '$nrordentra' and codservicio = '$codservicio1' and codestudio = '$codestudio1'" );

		while ( $rowcod = pg_fetch_array( $unro ) ) {
			$nroresul = $rowcod[ 'ultimo' ] + 1;
		}
		
		pg_query( $con, "INSERT INTO public.resultadomicroorganismo(
	nordentra, codservicio, nroestudio, idmuestra, nroresul, fechares, codmetodo, codumedida, codsector, codestudio, codmicroorg, resultado, codantibiogr, obs, codestudiobio)
	VALUES ('$nrordentra', '$codservicio1', '$nroestudio', 'XXX$nroresul', '$nroresul', '$fechares', '$codmetodo1', '$codumedida1', '$codsector1', '$codestudio1', '$codmicroorg', '$resultado', '$codantibiogr', '$obs', '$codestudiobio1')");
