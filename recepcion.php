...
<head>
	...
	<script>
		$( function () {
			...
			jQuery( "#listapacientes" ).jqGrid( {
				url: 'datosenviado.php?codservicior=<?php echo $codservicior; ?>&desde=<?php echo $desde; ?>&hasta=<?php echo $hasta; ?>',
				datatype: 'json',
				mtype: 'GET',
				loadonce: true,
				height: 330,
				...
				colNames: [ 'Nro. Agrupado', 'Nro. Orden', 'Nombres y Apellidos', 'Tipo Muestra', 'Estado', 'Actual Establecimiento', '' ],
				colModel: [
		...
		function gridReload() {
			var codservicior = jQuery( "#codservicior" ).val();
			var grupo = jQuery( "#grupo" ).val();
			var hasta = jQuery( "#hasta" ).val();
			var desde = jQuery( "#desde" ).val();

			jQuery( "#listapacientes" ).jqGrid( 'setGridParam', {
				url: "datosenviado.php?codservicior=" + codservicior + "&grupo=" + grupo + "&desde=" + desde + "&hasta=" + hasta,
				datatype: 'json'
			} ).trigger( "reloadGrid" );

		/*	jQuery.ajax({
						url:'buscarenvios.php',
						type:'POST',
						dataType:'json',
						data:{codservicior:codservicior, hasta:hasta, desde:desde, grupo:grupo}
					}).done(function(respuesta){

						  jQuery("#CantMuestra").text(respuesta.cantmuestra);
		...
		function RechazarAgrupamiento()
		{
			var codservicior = jQuery( "#codservicior" ).val();
			var grupo	       = jQuery( "#grupo" ).val();
			var codrechazo	 = jQuery( "#codrechazo1" ).val();

			var mensaje      = "";

			var s = jQuery("#listapacientes").jqGrid('getGridParam','selarrrow'), selnordentra = [],selcodestudio= [];
			...
			if ( codservicior == "" )
			{
				...
				jQuery("#codservicior").css("background-color", "yellow");

			}
			else
			{
				jQuery("#codservicior").css("background-color", "white");
			}

			if ( codrechazo == "" )
			{
				mensaje += '- Debe tener un Motivo de Rechazo!\n';
			...
			else
			{
				if(cantidad != 0)
				{
						$.each(s,function()
						{
							...
							selcodestudio.push(jQuery("#listapacientes").jqGrid('getCell',this,'codestudio'));

						});

					jQuery.ajax({
							url:'nuevo_rechazo.php',
							type:'POST',
							dataType:'json',
							data:{codservicior:codservicior, nordentra:selnordentra, codestudio:selcodestudio, grupo:grupo, codrechazo:codrechazo, cantidad:cantidad}
						}).done(function(respuesta){
							...
							if(respuesta.grupo != 0)
							{
							   ...
							   jQuery("#pacientes_detallesButton1").removeAttr('disabled');
								...
								swal({
										title: "Datos Rechazado Exitosamente!",
										text: "",
										type: "success"
								}).then(function() {

										window.open('impresion_recepcion.php?grupo='+respuesta.grupo, '_blank');
		...
		function ListaRechazo()
		{
			$(document).ready(function() {

			var data = {};
			$("#listarechazo option").each(function(i,el) {
			   data[$(el).data("value")] = $(el).val();
			});
			console.log(data, $("#listarechazo option").val());

				var value = $('#codrechazo').val();

				$('#codrechazo1').val($('#listarechazo [value="' + value + '"]').data('value'));
			});
	...
	<script>
		$(document).ready(function(){
            ...
            //obtengo la data
            var codservicior = jQuery( "#codservicior" ).val();
						var s = jQuery("#listapacientes").jqGrid('getGridParam','selarrrow'), selnordentra = [],selcodestudio= [],selgrupo= [];
						var cantidad=s.length;
						var mensaje      = "";
						...
						else
						{
							...
							if(cantidad != 0)
							{
									$.each(s,function()
									{
										selnordentra.push(jQuery("#listapacientes").jqGrid('getCell',this,'nordentra'));
										selcodestudio.push(jQuery("#listapacientes").jqGrid('getCell',this,'codestudio'));
										selgrupo.push(jQuery("#listapacientes").jqGrid('getCell',this,'grupo'));
									...
									$.ajax({
										 type: "POST",
										 url: "nuevo_recepcion.php",
										 data:{
											 codservicior:codservicior, nordentra:selnordentra, codestudio:selcodestudio, grupo:selgrupo, cantidad:cantidad
										 },
										 dataType:'json',
												 ...
												 if(data.message != 0)
												 {
														  ...
														  swal({
																 ...
																 type: "success"
																		 ...
																		 window.open('impresion_recepcion.php?grupo='+data.message, '_blank');
																		 window.location = "recepcion.php";
																});}, 1000);
...
<body onLoad="gridReload();">
	...
	<form name="formu" id="formu" method="post">
		<div id="wb_pacientesLayoutGrid1">
			<div id="pacientesLayoutGrid1">
				<div class="row">
					...
					<div class="col-2">
						<hr id="Line4">
						<input type="text" id="grupo" name="grupo" value="<?php echo $grupo; ?>" spellcheck="false" onKeyUp="gridReload()">
						<hr id="Line5">
					</div>
				...
				<div class="row">
					...
					<div class="col-2">
						<hr id="Line4">
							...
							<select name="codservicior" size="1" id="codservicior" onChange="gridReload()">
								<option value=""></option>
								<?php
				...
				<div class="row">
					<div id="page1Layer1_Container">
						...
						if ( $v_212 == 2 || $v_212 == 3 ) {
							...
							echo '<button type="button" class="btn btn-primary btn-lg" id="Button10">Recepcion</button>';
							echo '&nbsp;&nbsp;&nbsp;';
						...
						if ( $v_213 == 2 || $v_213 == 3 ) {
							...
							echo '<button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#myModal" id="button">
							  Rechazar Recepcion
							</button>';
		...
		<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" >
			...
			<div class="modal-content">
				...
				Motivo de Rechazo
				<input type="text" name="codrechazo" id="codrechazo" list="listarechazo" style="width: 80%"  onkeypress="return validarcar(event)" onChange="ListaRechazo()" autofocus>

				 <input type="hidden" name="codrechazo1" id="codrechazo1" value="<?php echo $codrechazo; ?>" >

				</div>
				<div class="modal-footer">
				<button type="button" class="btn btn-primary" onclick="RechazarAgrupamiento()">Guardar Datos</button>
				</div>
			</div>
			</div>
		</div>
	...
	<datalist id="listarechazo">
	  <?php
			$tabla_dpto = pg_query($link, "select * from motivorechazo");
			echo '<option data-value = "" value = ""></option>';
			while($depto = pg_fetch_array($tabla_dpto))
			{
				echo '<option data-value = "'.$depto['codrechazo'].'" value = "'.$depto['codrechazo']."- ".$depto['nomrechazo'].'"></option>';
			}
	?>
	</datalist>
