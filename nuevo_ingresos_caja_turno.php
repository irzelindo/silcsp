...
$nomyape  = $row1["pnombre"].' '.$row1["snombre"].' '.$row1["papellido"].' '.$row1["sapellido"];



$query = "select * from apertura_cierre_caja where id = (select max(id) from apertura_cierre_caja where codusu = '$codusu' and tipo = '1' and fechaapertura = '$fecha')";
...
function ValidarFormulario($form) {
	...
	if ( $mensaje == "" ) {
		...
		$unro = pg_query( $con, "select max(CAST(coalesce(nroingreso, '0') AS integer)) as ultimo from ingresocaja" );

		while ( $rowcod = pg_fetch_array( $unro ) ) {
			$nroingreso = $rowcod[ 'ultimo' ] + 1;
		}

		if($formapago != 7)
		{
			$unro1 = pg_query( $con, "select max(n_recibo) as ultimo from config_gral where codservicio = '$codservicio1'" );

			while ( $rowcod1 = pg_fetch_array( $unro1 ) ) {
				$nrorecibo = $rowcod1[ 'ultimo' ] + 1;
			}
		}
		else
		{
			$unro1 = pg_query( $con, "select max(ne_recibo) as ultimo from config_gral where codservicio = '$codservicio1'" );
		...
		if ( $countlc == 0 ) {
			...
			pg_query( $con, "INSERT INTO public.ingresocaja(
	nroingreso, nrorecibo, nroreciboser, fecha, hora, formapago, otrafp, estado, nomyape, nropaciente, codcaja, codusu, codservicio, idapertura)
	VALUES ('$nroingreso', '$nrorecibo', '$nroreciboser', '$fecha', '$hora', '$formapago', '$otrafp', '1', '$nomyape', '$npaciente', '$codcaja1', '$codusu', '$codservicio1', '$idapertura')");
			...
			if($formapago != 7)
			{
				pg_query( $con, "UPDATE config_gral
							SET n_recibo = '$nrorecibo'
							WHERE codservicio='$codservicio1'" );
			}
			else
			{
				pg_query( $con, "UPDATE config_gral
							SET ne_recibo = '$nrorecibo'
							WHERE codservicio='$codservicio1'" );
			}
			...
			$accion = "Ingreso por Caja: Inserta-Reg.: Nro. Ingreso: " . $nroingreso;
			$terminal = $_SERVER[ 'REMOTE_ADDR' ];
			$a = archdlog( $_SESSION[ 'codusu' ], $codopc, $fecha1, $hora, $accion, $terminal );
			// Fin grabacion de registro de auditoria
			
			$respuesta->redirect("modifica_ingresos_caja_turno.php?nroingreso=$nroingreso&nroturno=$nroturno&codarea=$codarea&codturno=$codturno&mensage=2");
			
		} else {
			pg_query( $con, "UPDATE ingresocaja
							SET nrorecibo='$nrorecibo', nroreciboser='$nroreciboser', fecha='$fecha', hora='$hora', formapago='$formapago', otrafp='$otrafp', nomyape='$nomyape', nropaciente='$nropaciente', codcaja='$codcaja1', codusu='$codusu', codservicio='$codservicio1', idapertura='$idapertura'
			...
			$fecha1 = date( "Y-n-j", time() );
			$hora = date( "G:i:s", time() );
			$accion = "Ingreso por Caja: Modificar-Reg.: Nro. Ingreso: " . $nroingreso;
			$terminal = $_SERVER[ 'REMOTE_ADDR' ];
			$a = archdlog( $_SESSION[ 'codusu' ], $codopc, $fecha1, $hora, $accion, $terminal );
			// Fin grabacion de registro de auditoria
		}
...
<head>
	...
	<script>
		$( function () {
			...
			jQuery( "#listaconceptos" ).jqGrid( {
				url: 'datosarancelescajaturno.php?nroingreso=<?php echo $nroingreso; ?>&nrorecibo=<?php echo $nrorecibo; ?>&nroreciboser=<?php echo $nroreciboser; ?>&nroturno=<?php echo $nroturno; ?>&codarea=<?php echo $codarea; ?>&codturno=<?php echo $codturno; ?>',
				datatype: 'json',
				mtype: 'GET',
				loadonce: true,
				height: 200,
				...
				colNames: ['Borrar', 'Nro.', 'C&oacute;digo Arancel', 'Concepto', 'Monto'],
				colModel: [ {
					name: 'borrar',
					width: 60,
					resizable: false,
	...
	<script language="JavaScript">
		...
		{ 
			...
			var codempresa 	= jQuery("#npaciente").val();

			if(formapago == 2 || formapago == 6)
			{
				window.open("nuevo_homebanking.php?nroingreso=" + nroingreso + "&formapago=" + formapago+ "&codempresa=" + codempresa);
			}
		...
		{ 
			var nroturno     = window.document.formu.nroturno.value;
			var codarea      = window.document.formu.codarea.value;
			var codturno     = window.document.formu.codturno.value;
			...
			swal({
					...
					.then((willDelete) => {
					  if (willDelete) 
					  {
						window.location = "eliminar_arancel_caja_turno.php?nroingreso=" + nroingreso + "&nrorecibo=" + nrorecibo+ "&nroreciboser=" + nroreciboser+ "&norden=" + norden + "&codarea=" + codarea + "&codturno=" + codturno + "&codservicio=" + codservicio;
					  } 
					  else 
					  {
			  ...
			  <div class="col-3">
				  <hr id="turnos_detallesLine7">
					<?php
						if ($v_1511==1) 
						{
						 echo '<input type="button" id="turnos_detallesButton3" onclick="openImprimir()" name="hcli" value="Imprimir Recibo">';
						}
					?>
			  </div>
		  </div>
