...
<!----------- PARA JSDATEPICKER  ---------->
<!-- <link rel="stylesheet" type="text/css" href="js/jsdatepicker/css/bootstrap-datepicker3.min.css">
<script src="js/jsdatepicker/js/bootstrap-datepicker.min.js"></script>
<script src="js/jsdatepicker/locales/bootstrap-datepicker.es.min.js"></script> -->
...
<script type="text/javascript">
	...
	$(function() {
      ...
      var cargar = {'Arbovirus':[{'mu':'08005','de':['800105','800106','800107','800108','800109','800110','800111','800112']},
                                 {'mu':'08009','de':['800113','800114','800115']},
                                 {'mu':'08010','de':['800117','800118','800119']}
                                ]
                   };
      // console.log(cargar['Arbovirus'])
	});
	...
	function Buscar(argument) {
		if ($("#FechaResLabD").val() != "" && $("#FechaResLab").val() != "") {
			LimpiarDatosAEnviar();
			$.ajax({
				type: 'POST',
				dataType: "json",
				url: 'informarResultadosDGVSajax.php',
				data:{ FechaD: $("#FechaResLabD").val(), FechaH: $("#FechaResLab").val(), op: argument, Enfermedad: $("#Enfermedad").val(), Muestra: $("#Muestra").val() },
			}).done(function(data) {
            $(".btnxls").attr("disabled", true)
				if ( data.arrResultado.length > 0 ) {
               ArResultados = data.arrResultado;
					$("#btnxls"+argument).attr("disabled", false)
					MatrizInformeDGVS(data.arrResultado,"AEnviar",function(argument) { });
				}else{
					$("#btnxls"+argument).attr("disabled", true)
         ...
         swal('MUESTRAS...', "Debe ingresar fecha desde y fecha hasta...");
      }
	}

	function xls(argument) {
		if ($("#FechaResLabD").val() != "" && $("#FechaResLab").val() != "") {
			var form = document.createElement("form");
			form.setAttribute("method", "post");
			form.setAttribute("action", "informeDGVSxls.php");
			form.setAttribute("hidden", true);
			
			var hiddenFieldD = document.createElement("input");
			hiddenFieldD.setAttribute("name", "FechaD");
			hiddenFieldD.setAttribute("value", $("#FechaResLabD").val() );

         var hiddenFieldH = document.createElement("input");
         hiddenFieldH.setAttribute("name", "FechaH");
         hiddenFieldH.setAttribute("value", $("#FechaResLab").val() );

         var hiddenFieldOP = document.createElement("input");
         hiddenFieldOP.setAttribute("name", "op");
         hiddenFieldOP.setAttribute("value", argument );
			
			form.appendChild(hiddenFieldD);
         form.appendChild(hiddenFieldH);
         form.appendChild(hiddenFieldOP);
   ...
   function MatrizInformeDGVS(argument,argument1,callback) {
		$("#tsw"+argument1).empty();
		$("#tabla"+argument1).hide();
		// console.log(argument);
		var ArrArgument = eval(argument);
		if ( ArrArgument.length > 0 ) {
         var $newtable = $("<table id='tbl_"+argument1+"' class='table table-condensed'>");
         var $newtablehead = $("<thead>");
         var Cabeza = "<th style='padding: 6px 6px !important;'>Item</th>"+
                   "<th style='padding: 6px 6px !important;'>Nro Orden</th>"+
                   "<th style='padding: 6px 6px !important;'>CI</th>"+
                   "<th style='padding: 6px 6px !important;'>Nombres</th>"+
                   "<th style='padding: 6px 6px !important;'>Apellidos</th>"+
                   "<th style='padding: 6px 6px !important;'>Nro.Muestra</th>"+
                   "<th style='padding: 6px 6px !important;'>Muestra</th>"+
                   // "<th style='padding: 6px 6px !important;'>Pedido para</th>"+
                   "<th style='padding: 6px 6px !important;'>Fecha Toma</th>"+
                   "<th class='text-center'></th>"+
                   // "<th class='text-center'></th>"+
                   // "<th>Nro. Laboratorio</th>"+
                   // "<th>CT</th>"+
                   "<th class='text-center'><input type='checkbox' id='chk_all' name='chk_all'></th>";
         $($newtablehead).append("<tr><th colspan='11' style='padding: 6px 6px !important;'>MUESTRAS</th></tr>");
         ...
         for (var i = 0; i < ArrArgument.length; i++) {
            var Total = "";
            // +" / "+ArrArgument[i]["cod_dgvs"]
            if (cedula != ArrArgument[i]["cedula"]) {
               Item++;
               var toTR   = "<td>"+Item+"</td>"+
                              "<td>"+ArrArgument[i]["nordentra"]+"</td>"+
                              "<td>"+ArrArgument[i]["cedula"]+"</td>"+
                              "<td>"+ArrArgument[i]["nombres"]+"</td>"+
                              "<td>"+ArrArgument[i]["apellidos"]+"</td>"+
                              "<td>"+ArrArgument[i]["nro_toma"]+"</td>"+
                              "<td>"+ArrArgument[i]["codtmuestra_desc"]+"</td>"+
                              "<td>"+ArrArgument[i]["fecharec_esp"]+"</td>"+
                              "<td style='padding: 0 0 0 0 !important;'><button type='button' class='btn btn-sm btn-primary' value='0' id='btn_"+ArrArgument[i]["nordentra"]+"' onclick='VerRE(\""+ArrArgument[i]["nordentra"]+"\")'><i class='fa fa-plus-square' aria-hidden='true'></i></button></td>"+
                              "<td class='text-center'><input type='checkbox' id='chk_"+ArrArgument[i]["nordentra"]+"' name='chk_"+ArrArgument[i]["nordentra"]+"'></td>";
               Total  = "<tr id='fila_"+ArrArgument[i]["cedula"]+"' style='border-top: 2px solid black;background-color: #EAFAF1;'>"+toTR+"</tr>";
               Total += "<tr><td id='deta_"+ArrArgument[i]["nordentra"]+"' colspan='11' style=';display:none'></td></tr>";
               cedula = ArrArgument[i]["cedula"];
            }
            $($newtablebody).append(Total);
         }
         ...
         $('[name="chk_all"]:checkbox').change(function() {
            ...
            if ( $(this).prop("checked") ) {
               $("[id^='fila_']").addClass("success");
            }else{
               $("[id^='fila_']").removeClass("success");
            }  
         });

         $("#chk_all").show()
         $('[name^="chk_"]:checkbox').not("#chk_all").change(function() {
            var ident = $(this).prop("id").split("_")[1];
            // console.log(ident)
            if ( $(this).prop("checked") ) {
               $("#fila_"+ident).addClass("success");
            }else{
               $('#fila_'+ident).removeClass("success");
            }
         });         
			callback();
		}else {
   ...
   function VerRE(argument) {
      if ( $("#btn_"+argument).val() == '0' ) {
         var internal = $.grep(ArResultados, function (row) {
            return row["nordentra"] === argument;
         });
         $("#btn_"+argument).val('1');
         $("#btn_"+argument).html("<i class='fa fa-minus-square' aria-hidden='true'></i>");
         var detalle = "<tr style='background-color: aliceblue;'>"+
                       // "<th style='padding-top: 0px;padding-bottom: 0px;'>Laboratorio</th>"+
                       // "<th style='padding-top: 0px;padding-bottom: 0px;'>F.Recepcion</th>"+
                       // "<th style='padding-top: 0px;padding-bottom: 0px;'>Nro.Lab</th>"+
                       "<th style='padding-top: 0px;padding-bottom: 0px;'>Enfermedad</th>"+
                       "<th style='padding-top: 0px;padding-bottom: 0px;'>Estudio</th>"+
                       "<th style='padding-top: 0px;padding-bottom: 0px;'>Resultado</th>"+
                       "<th style='padding-top: 0px;padding-bottom: 0px;'>F.Resultado</th>"+
                       "<th style='padding-top: 0px;padding-bottom: 0px;'>Enviado</th>"+
                       "</tr>";
         var $newtableRe = $("<table id='tbl_Resultados' class='table table-condensed'>");
         var $newtableheadRe = $("<thead>");
         $($newtableheadRe).append("<tr><th colspan='1' style='padding: 6px 6px !important;'>Nro. Laboratorio: "+argument+"</th>"+
                                       "<th colspan='1' style='padding: 6px 6px !important;'>Laboratorio: LCSP</th>"+
                                       "<th colspan='2' style='padding: 6px 6px !important;'>Recepcion: "+internal[0]["fecharec_esp"]+"</th></tr>");
         $($newtableheadRe).append( detalle );
         $($newtableRe).append($newtableheadRe);
         var $newtablebodyRe = $("<tbody>");
         for (var i = 0; i < internal.length; i++) {
            var TrRe = "<tr class='tr_"+internal[i]["coddetermina"]+"'>";
            // TrRe += "<td style='padding-top:0px;'>LCSP</td>";
            // TrRe += "<td class='text-nowrap' style='padding-top:0px;'>"+internal[i]["fecharec_esp"]+"</td>";
            // $($newtrRe).append("<td class='text-nowrap' style='padding-top:0px;'>"+internal[i]["nordentra"]+"</td>");
            TrRe += "<td class='text-nowrap' style='padding-top:0px;'>"+internal[i]["codestudio_desc"]+"</td>";
            TrRe += "<td class='text-nowrap' style='padding-top:0px;'>"+internal[i]["coddetermina_desc"]+"</td>";
            TrRe += "<td class='text-nowrap' style='padding-top:0px;'>"+(internal[i]["resultado"] == '' ? (internal[i]["codresultado_desc"] != null ? internal[i]["codresultado_desc"] : 'Pendiente') : internal[i]["resultado"]) +"</td>";
            TrRe += "<td class='text-nowrap' style='padding-top:0px;'>"+(internal[i]["resultado"] == '' ? (internal[i]["codresultado_desc"] != null ? internal[i]["fechares_esp"] : 'Pendiente') : internal[i]["fechares_esp"]) +"</td>";
            TrRe += "<td class='text-nowrap' style='padding-top:0px;'>"+(internal[i]["envio_dgvs"] == 1 ? '<i class="fa fa-check" aria-hidden="true"></i>':'<i class="fa fa-ban" aria-hidden="true"></i>')+"</td>";
            TrRe += "</tr>";
            $($newtablebodyRe).append(TrRe);
         }
         $($newtableRe).append($newtablebodyRe);
         $("#deta_"+argument).append($newtableRe);
         $("#deta_"+argument).show();
      }else{
         $("#btn_"+argument).val('0');
         $("#btn_"+argument).html("<i class='fa fa-plus-square' aria-hidden='true'></i>");
         $("#deta_"+argument).empty();
         $("#deta_"+argument).hide();
      }
   }
   ...
   function EnviarDGVS(argument) {
      var aGuardar = [];
      var aGuardar = $( "input[name^='chk_']:checked" ).not("#chk_all");
      if ( aGuardar.length > 0 ) {
         var ToSend  = [];
         var NoEnvio = 0;
         var m       = 0;
         for (var i = 0; i < aGuardar.length; i++) {
            var pos = aGuardar[i].id.substr(4, 1000);
            var internal = $.grep(ArResultados, function (row) {
               return row["nordentra"] === pos && ($.trim(row["codresultado"]) != "" || $.trim(row["resultado"]) != "");
            });
            for (var k = 0; k < internal.length; k++) {
               ToSend.push(internal[k]);
            }
         }
         // console.log(ToSend)
         if ( NoEnvio == 0 ) {
            if ( ToSend.length > 0 ) {
               $.ajax({
                  type: 'POST',
                  dataType: "json",
                  url: 'https://dgvs.mspbs.gov.py/sistemas/api/LCSPRes',
                  data:{ LCSPRes: ToSend, Enfermedad: $("#Enfermedad").val() },
               }).done(function(data) {
                  if ( data.respuesta.length > 0 ) {
                     $.ajax({
                        type: 'POST',
                        dataType: "json",
                        url: 'informarResultadosRecibidosDGVSajax.php',
                        data:{ LCSPRes: data.respuesta, Enfermedad: $("#Enfermedad").val() },
                     }).done(function(data) {
                        if ( data.respuesta2.length > 0 ) {
                     ...
                     alert('Completado... No se realizo ninguna acción');
                  }
               })
               .fail(function(){})
               .always(function(){});
            }else{
               alert('Error...! No se encontro nada que enviar');
            }
         }else{
            $("input[name^='chk_']").prop('checked', false);
            for (var i = 0; i < ArrAEnviar.length; i++) {
               if ( ArrAEnviar[i]["Cantidad"] == "0" ) {
                  $("#chk_"+ArrAEnviar[i]["Codigo"]).prop('checked', true);
               }
            }
            // $.alert({title: 'Error...!',content: "Hay Codigos que no contienen muestras del dia<br/> Favor corregirlos o autocorregir con el boton autocorrección<br/> Verifique los marcados para ver si corresponden a la autocorrección"});
            // $("#bc_Autocorreccion").show();
         }
      }else{
         if ( aGuardar.length <= 0 ) {
            alert('Error...! Favor Seleccionar los datos a enviar');
         }
         // waitingDialog.hide();
      }
      alert("En proceso");
	...
	<div id="LayoutGrid3">
		<div class="row">
			<div class="col-1">
				<hr id="Line9"/>
				<div id="wb_Text1">
					<span style="color:#000000;font-family:Arial;font-size:13px;"><strong>USUARIO: </strong></span><span style="color:#FF0000;font-family:Arial;font-size:13px;"><strong><?php echo $elusuario;?></strong></span>
<!--                <span style="color:#000000;font-family:Arial;font-size:13px;"><strong><br><br></strong></span><span style="color:#808080;font-family:Verdana;font-size:16px;"><strong>ORDENES</strong></span><span style="color:#000000;font-family:Arial;font-size:13px;"><strong><br></strong><br />
					</strong></span> -->
				</div>
			</div>
		</div>
...
<!-- EMPIEZA -->
   <div class="col-xs-12" id="frm_fsarscov2" name="frm_fsarscov2" style="margin-top: 0px;padding: 0px;">
      <div class="page-header" style="margin-top: 0px;padding-top: 0px;">
         <h3 style="margin-top: 0px;padding-top: 0px;">Informar Resultados a la D.G.V.S.</h3>
      </div>
<!--       <div class="row">
         <div calss="col-sm-12">
      ...
      <div class="panel panel-default" id="bc_AEnviar">
         <div class="panel-heading"><i class="glyphicon glyphicon-user"></i> INFORMAR </div>
         <div class="panel-body" id="pb_AEnviar">
            <h3>Seleccione Enfermedad</h3>
            <div class="col-xs-6 col-sm-3 col-md-3" style="padding:0px">
               <select id="Enfermedad" name="Enfermedad" class="form-control" style="height: 30px;">
                  <option value='SD'>SD</option>
                  <option value='2'>Arbovirus</option>
                  <option value='9'>Viruela simica</option>
                  <option value='-1'>Secuenciacion SarsCov2</option>
               </select>
            </div>
            <div class="clearfix"></div>
            <h3>Fechas para informar</h3>
            <div class="row">
               <div class="col-xs-6 col-sm-3 col-md-2">
                  <label for="CodigoEnvio">Fecha Desde</label>
                  <input id="FechaResLabD" name="FechaResLabD" class="form-control" data-date-format="mm/dd/yyyy" placeholder="dd/mm/yyyy" style="width:83%;height: 20px;">
               </div>
               <div class="col-xs-6 col-sm-3 col-md-3">
                  <label for="CodigoEnvio">Fecha Hasta</label>
                  <div class="input-group">
                     <!-- <input id="FechaResLab" name="FechaResLab" class="js-datepicker form-control" type="text" data-date-format="dd/mm/yyyy" style="width:83%;height: 20px;" placeholder="dd/mm/yyyy"> -->
                     <input id="FechaResLab" name="FechaResLab" class="form-control" data-date-format="mm/dd/yyyy" placeholder="dd/mm/yyyy" style="width:83%;height: 20px;">
                  </div>
               </div>
               <div class="col-xs-6 col-sm-3 col-md-3">
                  <label for="Muestra">Muestras</label>
                  <select id="Muestra" name="Muestra" class="form-control" style="height: 30px;">
                     <option value='1'>RECEPCION SIL</option>
                     <option value='2'>RECEPCION DGVS</option>
                  </select>
               </div>
               <div class="col-sm-12">
                  <button class="btn btn-default" type="button" onclick="Buscar(1)">
                     <span class="glyphicon glyphicon-search" aria-hidden="true"> Fecha de Resultado</span>
                  </button>
                  <button type="button" class="btn btn-default hidden-print btnxls" id="btnxls1" onclick="xls(1)" disabled="disabled">
                     <i class="fa fa-file-excel-o" aria-hidden="true"></i>
                  </button>

                  <button class="btn btn-default" type="button" onclick="Buscar(2)">
                     <span class="glyphicon glyphicon-search" aria-hidden="true"> Fecha de Recepcion</span>
                  </button>
                  <button type="button" class="btn btn-default hidden-print btnxls" id="btnxls2" onclick="xls(2)" disabled="disabled">
                     <i class="fa fa-file-excel-o" aria-hidden="true"></i>
                  </button>

                  <button type="button" class="btn btn-default hidden-print" onclick="LimpiarDatosAEnviar()">
                     <span class="glyphicon glyphicon-remove" aria-hidden="true"> Reanudar</span>
                  </button>
                  <button type="button" class="btn btn-default hidden-print" onclick="EnviarDGVS()">
                     <span class="glyphicon glyphicon-saved" aria-hidden="true"> Enviar a la DGVS</span>
                  </button>
               </div>
            </div>
