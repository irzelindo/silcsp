...
@Header("Content-type: text/html; charset=utf-8");
   ...
   $sql1 = "select * from resultadomicroorganismo where codantibiogr = '$id'";
   $res1=pg_query($con,$sql1);
   $numeroRegistros1=pg_num_rows($res1);

   $sql2 = "select * from antibioticoantibiograma where codantibiogr = '$id'";
   ...
   if($numeroRegistros<=0)
      {
      $resultado1=pg_query($con,"DELETE FROM antibiogramas WHERE codantibiogr = '$id'"); 

      $resultado2=pg_query($con,"DELETE FROM antibioticoantibiograma WHERE codantibiogr = '$id'"); 
      ...
      if($resultado1==true && $resultado2==true)
           {
              ...
              $accion="Antibiogramas: Elimina-Reg.: ".$id;
              $terminal = $_SERVER['REMOTE_ADDR'];
              $a=archdlog($codusu,$codopc,$fecha,$hora,$accion,$terminal);
              
              // Fin grabacion de registro de auditoria
              header("Location: antibiogramas.php?mensage=1");	
           }
    	else
    	  {
      ...
      {		
      header("Location: antibiogramas.php?mensage=4"); //no se puede borrar
      }
?>
