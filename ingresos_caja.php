...
$sql = "select * from apertura_cierre_caja where id = (select max(id) from apertura_cierre_caja where codusu = '$codusu' and tipo = '1' and fechaapertura = '$fecha')";
...
x(document).ready(function() {
	x("#Buttonc").click(function(){
		if(jQuery("#Combobox1").val() != '')
		{
			window.location = "nuevo_ingresos_caja_detalle.php?codservicio="+jQuery("#Combobox1").val();
		}
		else
		{
			swal("Los campos Laboratorio responsable deben contener algun valor!", "", "warning");
		}
...
$(function() {
    ...
    jQuery("#listapacientes").jqGrid({
        url:'datosingresoscaja.php?codservicio=<?php echo $codservicio; ?>',
        datatype: 'json',
        mtype: 'GET',
    	loadonce:true,
        height: 200,
        ...
        colNames:['Ver', 'Anular','Nro. Ingreso', 'Fecha','Empresa','Nro. Exp. Homebanking', 'Total Ticket'],
        colModel:[
				{name:'ver', width:60, resizable:false, align:"center",sorttype:"int", editable: false, editoptions:{maxlength:"50"}, search: false},
			
				{name:'anular', width:60, resizable:false, align:"center",sorttype:"int", editable: false, editoptions:{maxlength:"50"}, search: false},
			
                {name:'nroingreso',index:'nroingreso', width:150, align:"center", editable: true, searchoptions: {attr: {maxlength: 10,size: 40,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
			
				{name:'fecha',index: "fecha", width: 110, align:"center", editable: true, searchoptions: {attr: {maxlength: 100,size: 80,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
			
                {name:'nomempresa',index: "nomempresa", width: 500, editable: true, searchoptions: {attr: {maxlength: 100,size: 80,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
			
			    {name:'nroexpediente',index:'nroexpediente', width:150, align:"center", editable: true, searchoptions: {attr: {maxlength: 10,size: 40,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}},
			
				{name:'monto',index:'monto', width:150, align:"right", editable: true, searchoptions: {attr: {maxlength: 10,size: 40,style: "width:auto;padding:1;max-width:100%;height:3em;float:left"}}}
                 ],
...
{ 
	...
	jQuery("#listapacientes").jqGrid('setGridParam',{url:"datosingresoscaja.php?codservicio="+codservicio+"&desde="+desde+"&hasta="+hasta,datatype:'json'}).trigger("reloadGrid");
         ...
         if ($v_151==2 || $v_151==3) 
            {
    	     	if($countlc != 0)
				{
					echo '<button type="button" id="Buttonc" class="btn btn-primary btn-lg">Agregar</button>';
				}
            }
         ?>
      <div class="jqGrid">
