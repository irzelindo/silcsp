...
$v_123 = $_SESSION['V_123'];

$nropaciente = $_GET['nropaciente'];

$fechatur = date("Y-m-d", time());
$horatur  = date("H:i");
...
$xajax->register( XAJAX_FUNCTION, 'GenerarOrden' );
$xajax->register( XAJAX_FUNCTION, 'ReasignarTurno' );
$xajax->register( XAJAX_FUNCTION, 'NavegadorOrden' );
$xajax->register( XAJAX_FUNCTION, 'ImprimirIndicacion' );
$xajax->register( XAJAX_FUNCTION, 'Perceptoria' );
...
function ValidarFormulario($form) {
	...
	if ( $horatur == "" ) {
		...
		$respuesta->Assign( "horatur", "style.backgroundColor", "yellow" );

	} else {
		$respuesta->Assign( "horatur", "style.backgroundColor", "white" );
	}
	...
	if ( $npaciente == "" ) {
		...
		$respuesta->Assign( "npaciente", "style.backgroundColor", "white" );
	}
	...
	if ( $codservicio == "" ) {
		...
		$respuesta->Assign( "codservicio", "style.backgroundColor", "white" );
	}

	if ( $codarea == "" ) {
		$mensaje .= '- Rellene el campo Area del Establecimiento!\n';

		$respuesta->Assign( "codarea", "style.backgroundColor", "yellow" );

	} else {
		$respuesta->Assign( "codarea", "style.backgroundColor", "white" );
	}

	if ( $codturno == "" ) {
		$mensaje .= '- Rellene el campo Tipo de Turno!\n';

		$respuesta->Assign( "codturno", "style.backgroundColor", "yellow" );

	} else {
		$respuesta->Assign( "codturno", "style.backgroundColor", "white" );
	}
	...
	if ( $mensaje == "" ) {
		...
		if ( $nrotur == "" ) {
			...
			$unro = pg_query( $con, "select max(CAST(coalesce(nroturno, '0') AS integer)) as ultimo from turnos where codservicio = '$codservicio'" );
            ...
            nroturno, codservicio, codarea, nropaciente, fechatur, horatur,
            urgente, asistio, reasignado, nroturreas, suspendido,
            obs, codturno)
    VALUES ('$nroturno', '$codservicio', '$codarea', '$npaciente', '$fechatur', '$horatur', '$urgente', '$asistio', '$reasignado', '$nroturreas', '$suspendido', '$obs', '$codturno')" );

			$respuesta->Assign( "pacientes_detallesButton1", "style.visibility", 'visible' );
			$respuesta->Assign( "pacientes_detallesButton2", "style.visibility", 'visible' );
			$respuesta->Assign( "pacientes_detallesButton3", "style.visibility", 'visible' );
			$respuesta->Assign( "turnos_detallesButton2", "style.visibility", 'visible' );

			$respuesta->Assign( "turnos_detallesButton5", "style.visibility", 'visible' );

			$respuesta->Assign( "turnos_detallesButton3", "style.visibility", 'visible' );

			$respuesta->Assign( "Button1", "style.visibility", 'visible' );

			$respuesta->Assign( "nrotur", "value", $nroturno);


			$respuesta->redirect("modifica_asignar_turnos_pacientes.php?nroturno=$nroturno&codservicio=$codservicio&mensage=2");
			...
			$accion = "Turno: Inserta-Reg.: Nro. Turno: " .$codservicio.$nroturno;
			$terminal = $_SERVER[ 'REMOTE_ADDR' ];
			$a = archdlog( $_SESSION[ 'codusu' ], $codopc, $fecha1, $hora, $accion, $terminal );
			// Fin grabacion de registro de auditoria
		} else {
			pg_query( $con, "UPDATE turnos
   							SET codservicio='$codservicio', codarea='$codarea', nropaciente='$npaciente', fechatur='$fechatur',
       						horatur='$horatur', urgente='$urgente', asistio='$asistio', obs='$obs', codturno='$codturno'
		 					WHERE nroturno = '$nrotur' and codservicio = '$codservicio'" );

			$respuesta->script('swal("Datos Modificado Exitosamente!", "", "success")');

			//$respuesta->redirect("modifica_pacientes.php?nropaciente=$npaciente");
			...
			$fecha1 = date( "Y-n-j", time() );
			$hora = date( "G:i:s", time() );
			$accion = "Turnos: Modificar-Reg.: Nro. Turno: ".$codservicio.$nrotur;
			$terminal = $_SERVER[ 'REMOTE_ADDR' ];
			$a = archdlog( $_SESSION[ 'codusu' ], $codopc, $fecha1, $hora, $accion, $terminal );
			// Fin grabacion de registro de auditoria
		}
...
function GenerarOrden($form) {
	...
	$sql = "select * from estrealizar where nroturno = '$nrotur' and codservicio = '$codservicio1'";
	...
	if ($countlc == 0)
	{
		$respuesta->script('swal("Datos", "Debe Agregar al menos un Estudio, para Generar la Orden de Trabajo!", "warning")');
	}
	else
	{
		while ($row = pg_fetch_array($res))
		{
			...
			{
				$unro = pg_query($con, "select max(CAST(coalesce(nordentra, '0') AS integer)) as ultimo from ordtrabajo where codservicio = '$codservicio1'");

				while ($rowcod = pg_fetch_array($unro))
				{
					$nordentra = $rowcod[ 'ultimo' ] + 1;
				}

				pg_query($con, "UPDATE estrealizar
										SET nordentra='$nordentra'
										WHERE nroturno = '$nrotur' and codservicio = '$codservicio1'");

				pg_query($con, "INSERT INTO ordtrabajo(
				nordentra, codservicio, nropaciente, fecharec, horarec, nroturno, codusu, codservrem)
				VALUES ('$nordentra', '$codservicio1', '$npaciente', '$fecharec', '$horarec', '$nrotur', '$codusu', '$codservicio1')" );
				...
				if($conresul == 0)
				{
					...
					if($condeter != 0)
					{
						...
						nordentra, codservicio, nroestudio, idmuestra, nroorden, codumedida, codsector, codestudio, coddetermina, codestado, codmetodo, envio_dgvs)
						VALUES ('$nordentra', '$codservicio1', '$nroestudio', 'XXX$nroorden', '$nroorden', '$codumedida', '$codsector', '$codestudio', '$coddetermina', '001', '$codmetodo', 0)" );
						}
					}
				...
				$fecha1 = date( "Y-n-j", time() );
				$hora = date( "G:i:s", time() );
				$accion = "Orden Trabajo: Insertar-Reg.: Nro. Orden: " . $codservicio1.$nordentra;
				$terminal = $_SERVER[ 'REMOTE_ADDR' ];
				$a = archdlog( $_SESSION[ 'codusu' ], $codopc, $fecha1, $hora, $accion, $terminal );
				// Fin grabacion de registro de auditoria
			}
		...
		if($band == 1)
		{
			$respuesta->script('swal("Orden de Trabajo Generada Exitosamente!", "", "success")');

			$respuesta->script("xajax_GenerarMuestra(xajax.getFormValues('formu'))");
		}
		else
		{
			$respuesta->script('swal("La Orden de Trabajo ya fue Generada!", "", "warning")');
		}
...
function ReasignarTurno($form) {
	...
	$res1 	   = pg_query($con, $sql1);
	$cantorden = pg_num_rows($res1);

	if($cantorden == 0)
	{
		$respuesta->redirect("reasignar_turnos.php?nropaciente=$npaciente&nroturno=$nrotur&codservicio=$codservicio1&codarea=$codarea1&codturno=$codturno1&indica=1");
	}
	else
	{
		$respuesta->script('swal("No se puede Reasignar Turno, ya existe Nro. de Orden Generada!", "", "warning")');
	}

	return $respuesta;
}

function NavegadorOrden($form) {
	extract($form);
	...
	$nordentra 	= $rowp["nordentra"];

	if($nordentra != '')
	{
		$respuesta->redirect("navegador_ordenes.php?nordentra=$nordentra&codservicio=$codservicio1&nroturno=$nrotur&indica=1");
	}
	else
	{
		$respuesta->script('swal("Falta generar el Nro. de Orden!", "", "warning")');
	}

	return $respuesta;
}

function ImprimirIndicacion($form) {
	extract($form);

	$respuesta = new xajaxResponse();
	$respuesta->setCharacterEncoding( 'utf-8' );

	$respuesta->Script("ImprimirIndicacion($nrotur, '$codservicio')");

	return $respuesta;
}

function Perceptoria($form) {
	extract($form);
	...
	$sql1 = "select * from ingresocaja where nropaciente = '$npaciente' and codservicio = '$codservicio1' and fecha = '$fechatur'";

	$res1 	 = pg_query($con, $sql1);
	$countlc = pg_num_rows( $res1 );
	$rowp 	 = pg_fetch_assoc($res1);

	if($countlc == 0)
	{

		$respuesta->redirect("nuevo_ingresos_caja_turno.php?nropaciente=$npaciente&codservicio=$codservicio1&nroturno=$nrotur&codarea=$codarea1&codturno=$codturno1");
	}
	else
	{
		$respuesta->script('swal("Recibo ya generado!", "", "warning")');
	}
	...
	if ($mensaje == "")
	{
		if($numeroRegistros1 == 0)
		{
			...
			if($nordentra != '')
            {
        		...
        		VALUES ('$nroestudio', '$codservicio1', '$codarea1', '$npaciente', '$fechatur', '$horatur', '$codestudio1', '1', '$nrotur', '$codtmuestra', '$nordentra', '$fecharec', '$horarec', '$codusu', '$codservicio1', '$codservicio1')" );
            }
            else
            {
                pg_query( $con, "INSERT INTO estrealizar(
        		...
        		VALUES ('$nroestudio', '$codservicio1', '$codarea1', '$npaciente', '$fechatur', '$horatur', '$codestudio1', '1', '$nrotur', '$codtmuestra', '$codusu', '$codservicio1', '$codservicio1')" );
            }
			...
			$fecha1 = date( "Y-n-j", time() );
			$hora = date( "G:i:s", time() );
			$accion = "Turnos-Estudios: Insertar-Reg.: Estudio: " . $codestudio1;
			$terminal = $_SERVER[ 'REMOTE_ADDR' ];
			$a = archdlog( $_SESSION[ 'codusu' ], $codopc, $fecha1, $hora, $accion, $terminal );
			// Fin grabacion de registro de auditoria
		}
...
function GenerarMuestra($form)
{
	...
	$querymuestra = "select  distinct e.codestudio,
			...
			and   er.nroturno  = '$nrotur'
			and   er.codservicio= '$codservicio'";

	$resultmuestra = pg_query( $con, $querymuestra );

	$query2 = "select * from ordtrabajo where nroturno = '$nrotur' and codservicio = '$codservicio'";
	...
	while ( $rowmuestra = pg_fetch_array( $resultmuestra ) )
	{
		...
		$codestudioant  = $row[ "codestudio" ];
		$codsectorant   = $row[ "codsector" ];
		$codtmuestraant = $row[ "codtmuestra" ];
		$nromuestraant  = $row[ "nromuestra" ];

		if($count == 0)
		{
			$nromuestranew = $nromuestranew + 1;
		}
		else
		{
			if($codtmuestraant != $codtmuestra)
			{
				$unro = pg_query( $con, "select COALESCE(max(nromuestra), '0') as ultimo from estrealizar where codservicio = '$codservicio'" );

				while ( $rowcod = pg_fetch_array( $unro ) ) {
					$nromuestranew = $rowcod[ 'ultimo' ];
				}
		...
		pg_query($con, "UPDATE estrealizar er
								...
								WHERE er.nroturno = '$nrotur' and er.codservicio = '$codservicio' and codestudio = '$codestudio' and codtmuestra = '$codtmuestra' and er.nromuestra is null");
	}
...
<head>
	...
	$(document).ready(function() {
		$("#Button1").click(function(){
			if(jQuery("#codservicio").val() != '' && jQuery("#nrotur").val() != '')
			{
				window.location.href="nuevo_asignar_turnos_estudios.php?nroturno=" + jQuery("#nrotur").val() + "&codservicio=" + jQuery("#codservicio").val();
			}
			else
			{
				  swal("Datos", "Los campos Establecimiento de Salud y El Nro. Turno deben contener algun valor!", "warning");
			}
		});
	...
	<script>
		$( function () {
			...
			jQuery( "#listapacientes" ).jqGrid( {
				url: 'datospacientesestudiosasignar.php?nroturno=999&codservicio=999',
				datatype: 'json',
				mtype: 'GET',
				loadonce: true,
				height: 200,
	...
	<script language="JavaScript">
		...
		function ImprimirIndicacion(code, cod)
		{
			window.open('impresion_indicacion.php?nroturno=' + code + '&codservicio=' + cod, '_blank');
		}
		...
		function confirmacion(code, cod, nro, est)
		{
			swal({
				...
				.then((willDelete) => {
				  if (willDelete)
				  {
					window.location = "eliminar_asignar_turnos_estudio.php?nordentra=" + code + "&nroestudio="+ cod + "&nroturno="+ nro+ "&codservicio="+ est;
				  }
				  else
				  {
		...
		function gridReload()
		{
			var nroturno 	= jQuery("#nrotur").val();
			var codservicio = jQuery("#codservicio1").val();

			jQuery("#listapacientes").jqGrid('setGridParam',{url:"datospacientesestudiosasignar.php?nroturno="+nroturno+"&codservicio="+codservicio,datatype:'json'}).trigger("reloadGrid");
			...
			<div class="row">
			  <div class="col-3">
				  <div id="wb_pacientes_detallesText2">
						<span style="color:#696969;font-family:Verdana;font-size:30px;"><strong>Nro. de Turno:</strong></span>
				</div>
					<hr id="pacientes_detallesLine5">
			  </div>
				<div class="col-4">
					<input type="text" id="nrotur" name="nrotur" value="<?php echo $nroturno; ?>" readonly style="font-size: 30px; background-color: #DCDCDC">
		...
		<div id="turnos_detallesLayoutGrid5">
			<div class="row">
				...
				<div class="col-2">
					...
					<input type="date" id="fechatur" name="fechatur" value="<?php echo $fechatur; ?>" spellcheck="false" readonly style="background-color: #DCDCDC">
...
<div id="wb_turnos_detallesLayoutGrid5">
		<div id="turnos_detallesLayoutGrid5">
			<div class="row">
				...
				<div class="col-2">
					<hr id="turnos_detallesLine14">
					<input type="time" id="horatur" name="horatur" value="<?php echo $horatur; ?>" readonly style="background-color: #DCDCDC">
					<hr id="turnos_detallesLine15">
				</div>
			</div>
		</div>
	...
	<div id="usuarios_detallesLayoutGrid7">
			<div class="row">
				...
				<div class="col-2">
					...
					<select name="codservicio" size="1" id="codservicio" autofocus></select>
		...
		<div id="turnos_detallesLayoutGrid1">
			<div class="row">
				...
				<div class="col-2">
					...
					<select name="codarea" size="1" id="codarea"></select>
                        <script type="text/javascript">
							$(document).ready(function() {
		...
		<div id="courier_detallesLayoutGrid2">
			<div class="row">
				...
				<div class="col-2">
					...
					<select name="codturno" size="1" id="codturno"></select>
                        <script type="text/javascript">
							$(document).ready(function() {
		...
		<div id="pacientes_detallesLayoutGrid11">
			<div class="row">
				...
				<div class="col-2">
					...
					<input type="button" id="pacientes_detallesButton2" onclick="xajax_GenerarOrden(xajax.getFormValues('formu'));" name="hcli" value="Generar Ordenes de Trabajo">
				...
				<div class="col-1">
					...
					<?php
						if ($v_121==2 || $v_121==3)
						{
						 echo '<input type="button" id="pacientes_detallesButton1" onclick="xajax_ReasignarTurno(xajax.getFormValues(formu));" name="reasignar" value="Reasignar Turnos">';
						}
					?>
				...
				<div class="col-2">
				  <hr id="turnos_detallesLine8">
					<?php
						if ($v_123==1)
						{
						 echo '<input type="button" id="turnos_detallesButton5" onclick="xajax_ImprimirIndicacion(xajax.getFormValues(formu));" name="hcli" value="Imprimir Indicaciones">';
						}
					?>
				...
				<div class="col-3">
					...
					<input type="button" id="pacientes_detallesButton3" onclick="xajax_Perceptoria(xajax.getFormValues('formu'));" name="turnos" value="Perceptoria">
				</div>

				<div class="col-3">
					<hr id="turnos_detallesLine7">
					<input type="button" id="turnos_detallesButton3" onclick="xajax_NavegadorOrden(xajax.getFormValues('formu'));" name="hcli" value="Navegador de Ordenes">
				</div>
			</div>
		</div>
	</div>
			...
			<div class="row">
				<div class="col-1">
					...
					<div id="wb_sintomas_detallesText1">
						<span style="color:#FF0000;font-family:Arial;font-size:13px;">[&nbsp;<a href="#" onclick="window.location.href='modifica_pacientes.php?nropaciente=<?php echo $nropaciente; ?>'"> VOLVER </a>&nbsp;]</span>
...
{
	echo "<script type=''>
	 swal('Error', 'NO se puede borrar, pues otros datos dependen de este registro !!!', 'error');
     </script>";
}
?>
</html>
